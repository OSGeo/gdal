/******************************************************************************
 * $Id: VRC.cpp,v 1.274 2021/04/28 15:38:13 werdna Exp werdna $
 *
 * Project:  GDAL
...
CPL_CVSID("$Id: VRC.cpp,v 1.274 2021/04/28 15:38:13 werdna Exp werdna $")
#pragma clang diagnostic push


CPL_C_START
void   GDALRegister_VRC(void);
...
static int PNGGetInt( void* base, int byteOffset )
{
...
static unsigned int PNGGetUInt( void* base, int byteOffset )
{
    return(static_cast<unsigned int>(PNGGetInt(base, byteOffset)));
}


static
int PNGReadInt(VSILFILE *fp)
{
    unsigned char buf[4]={};
    // int ret =
    VSIFReadL(buf, 1, 4, fp);
    return PNGGetInt(buf, 0);
}
static
...
#else
    return(static_cast<unsigned int>(PNGReadInt(fp)));
#endif
}
...
int PNGCRCcheck(VRCpng_data *oVRCpng_data, unsigned long nGiven)
{
    ...
    if (nLen > static_cast<unsigned long int> (oVRCpng_data->length)
        || nLen > static_cast<unsigned long int> (1ll<<31)
        ) {
    ...
    unsigned long nFileCRC = 0xffffffff & static_cast<unsigned long>
        ( PNGGetInt(oVRCpng_data->pData,
                   static_cast<int>
                    (static_cast<unsigned long>(oVRCpng_data->current)
                     + nLen)) );
    if (nGiven == nFileCRC) {
        CPLDebug("Viewranger PNG",
    ...
    papoOverviewBands(papoOverviewBandsIn)
{
    ...
    nRasterXSize = poVRCDS->nRasterXSize >> (1+nThisOverview);
    nRasterYSize = poVRCDS->nRasterYSize >> (1+nThisOverview);
    ...
    if (poVRCDS->nMagic == vrc_magic_metres
        ...
        {
            unsigned int rounding = (1<<(1+nThisOverview)) -1;
        ...
        if (papoOverviewBands != nullptr) {
            ...
            if (nOverviewCount>=0) {
                papoOverviewBands = static_cast<VRCRasterBand **>
                    (CPLCalloc(sizeof(void*), static_cast<size_t>(nOverviewCount+1)));
                if (papoOverviewBands==nullptr) {
                    // Seen Fri 24 Jul 2020
                    CPLError(CE_Failure, CPLE_OutOfMemory,
...
#if 0 // Trying "packed DMS"
        dLeft   = CPLPackedDMSToDec(0.09*dLeft);
        dRight  = CPLPackedDMSToDec(0.09*dRight);
        dTop    = CPLPackedDMSToDec(0.09*dTop);
        dBottom = CPLPackedDMSToDec(0.09*dBottom);
#endif // Trying "packed DMS"
...

{
    ...
    if( nMagic == vrc_magic_metres ) {
        ...
        CPLDebug("Viewranger", "VRC36 file %s partially supported",
                 poOpenInfo->pszFilename);
        if (!b64k1) {
            CPLDebug("Viewranger",
                     "VRC file %s - limited support for unusual third long (not x10001)",
...
unsigned int* VRCDataset::VRCGetTileIndex( unsigned int nTileIndexStart )
{
    ...
    unsigned int *anNewTileIndex = static_cast<unsigned int *>
...
/* -------------------------------------------------------------------- */
    ...
    VRCDataset *poDS = new VRCDataset();
...
#undef VRCpszTAGlen
    ...
    if (static_cast<unsigned long>(0.5+10000.0*poDS->dfPixelMetres) != poDS->nScale )
    {
...
#undef VRC_DANGEROUS_TILE_INDEX
            ...
            for (int jj=0; jj<poDS->tileYcount; jj++) {
                for (int ii=0; ii<poDS->tileXcount; ii++) {
                    ...
                    CPLDebug("Viewranger",
                             "\t(%d,%d) = %d=x%08x",
                             ii, jj, nValue, nValue);
                    (void) nValue; // CPLDebug dosn't count as "using" a variable
                }
            }
        }
    ...
    // CPLDebug("Viewranger", "Datum: %s",poDS->sDatum.c_str());
...
/* -------------------------------------------------------------------- */
    ...
    if (getenv("VRC_MAX_SIZE")!=nullptr) {
        long long nMaxSize = atoll(getenv("VRC_MAX_SIZE"));
    ...
    if (!fSlowFile) {
        ...
        for (int i=1; i<=nMyBandCount; i++) {
            VRCRasterBand *poBand =
                new VRCRasterBand( poDS, i, -1, 6, nullptr); // here 11 June 2018
                        ...
                        )
{
    ...
    *********************************************************************/
    ...
    memcpy(static_cast<void*>(&oVRCpng_data.pData[oVRCpng_data.current]),
           aVRCHeader, 17 );
    int nPNGwidth=PNGGetInt(aVRCHeader, 0);
    *pPNGwidth=nPNGwidth;
    int nPNGheight=PNGGetInt(aVRCHeader, 4);
    *pPNGheight=nPNGheight;

    if (nPNGwidth<0 || nPNGheight<0) {
        CPLDebug("Viewranger PNG",
                 "negative PNG tile %d x %d (VRC tile %d,%d)",
    ...
    png_byte *pbyPNGbuffer = static_cast<unsigned char *>
        (VSIMalloc3(3,
    ...
    for (int ii=0; ii<3*nPNGwidth*nPNGheight; ii++) {
        // apc_row_pointers.data()[ii][jj] = 0;
        pbyPNGbuffer[ii] = nVRCNoData; // werdna July 14 2020
    }

    apc_row_pointers.reserve(static_cast<size_t>(nPNGheight));
    for (int ii=0; ii<nPNGheight; ii++) {
        apc_row_pointers.data()[ii] =
            &((static_cast<GByte*>(pbyPNGbuffer))[3*nPNGwidth*ii]);
    }
    png_set_rows(png_ptr, info_ptr, apc_row_pointers.data());

    unsigned char nPNGdepth = static_cast<unsigned char>(aVRCHeader[8]);
    unsigned char nPNGcolour = static_cast<unsigned char>(aVRCHeader[9]);
    unsigned char nPNGcompress = static_cast<unsigned char>(aVRCHeader[10]);
    unsigned char nPNGfilter = static_cast<unsigned char>(aVRCHeader[11]);
    unsigned char nPNGinterlace = static_cast<unsigned char>(aVRCHeader[12]);
    ...
    case 0: // Gray
        break;
    case 2: // RGB
        if (nPNGdepth==8 ||nPNGdepth==16) {
            break;
        }
        // else fall through
        [[clang::fallthrough]];
        // [[fallthrough]]; // C++17 onwards
    case 3: // Palette
    ...
    unsigned nBytesRead = static_cast<unsigned>
    ...
    if (getenv("VRC_DUMP_PNG")) {
        unsigned int nEnvPNGDump = static_cast<unsigned int>
...
extern "C" void GDALRegister_VRC()
...
{    
    VRCDataset* poVRCDS = static_cast<VRCDataset*>(poDS);
    if (poVRCDS == nullptr) {
        CPLDebug("VRC",
                             ...
                             void *pImage)
{
    ...
    int nPNGXcount = VRReadInt(fp);
    int nPNGYcount = VRReadInt(fp);
    int pngXsize  = VRReadInt(fp);
    int pngYsize  = VRReadInt(fp);

    if (nPNGXcount <=0) {
        CPLDebug("Viewranger",
                 "nPNGXcount %d zero or negative in tilenum %d",
                 nPNGXcount, tilenum );
        return;
    }
    if (nPNGYcount <=0) {
        CPLDebug("Viewranger",
                 "nPNGYcount %d zero or negative in tilenum %d",
                 nPNGYcount, tilenum );
        return;
    }
    if (pngXsize <=0) {
        CPLDebug("Viewranger",
                 "pngXsize %d zero or negative in tilenum %d",
                 pngXsize, tilenum );
        return;
    }
    if (pngYsize <=0) {
        CPLDebug("Viewranger",
                 "pngYsize %d zero or negative in tilenum %d",
                 pngYsize, tilenum );
        return;
    }

    if ( nPNGXcount > nBlockXSize * nShrinkFactor
         || pngXsize > nBlockXSize * nShrinkFactor
    ...
    if ( nPNGYcount > nBlockYSize * nShrinkFactor
         || pngYsize > nBlockYSize * nShrinkFactor
    ...
    int nLeftCol=0;
    int nPrevPNGwidth=0;
    int nXlimit = MIN(nPNGXcount, nBlockXSize*nShrinkFactor);
    int nYlimit = MIN(nPNGYcount, nBlockYSize*nShrinkFactor);
    for (int loopX=0; loopX < nXlimit; loopX++) {
        int nRightCol=0;
        int nPrevPNGheight=0;
        int nBottomRow=nBlockYSize;
        int nTopRow;

        // for (int loopY=0; loopY < nPNGYcount; loopY++) {
        for (int loopY=nYlimit-1; loopY >= 0; loopY--) {
            int loop = (nYlimit-1-loopY) + loopX*nPNGYcount;
            ...
            if ( static_cast<VRCDataset *>(poDS)->nMagic == vrc_magic_metres) {
                ...
                if (pbyPNGbuffer) {
                    ...
                    if (getenv("VRC_DUMP_TILE")) {
                        unsigned int nEnvTile = static_cast<unsigned int>
                    ...
                    if (nPNGheight>=nBlockYSize*nShrinkFactor) {
                        // single tile block
                        if (nTopRow<0) {
                            CPLDebug("Viewranger",
    ...
    if (getenv("VRC_DUMP_TILE")) {
        unsigned int nPPMcount = static_cast<unsigned int>
            (strtol(getenv("VRC_DUMP_TILE"),nullptr,10));
 ...
 )
{
    ...
    // No. one of the Sweded or Finland Overviews has bands whihc flash on and off with this code. Retire it.

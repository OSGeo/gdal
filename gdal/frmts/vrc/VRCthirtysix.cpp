/******************************************************************************
 * $Id: VRCthirtysix.cpp,v 1.18 2021/03/25 11:24:22 werdna Exp $
 *
...
{
    size_t count;
    for (count=0; count<nLen && data[count]==pattern[count]; count++) {
    }
    return count;
}
                                ...
                                void *pImage)
{
    ...
    // Since the tiles are divided into subtiles, this is not particularly informative
    // - one pixel per subtile would be better.
    int nXsize = poGDS->nRasterXSize;
    int nYsize = poGDS->nRasterYSize;
    CPLDebug("Viewranger", "\traster %d x %d tilenum %d",
             nXsize, nYsize, tilenum );
    if (block_xx != 0 || block_yy != 0 ) {
        CPLDebug("Viewranger",
    ...
    for (int i=0; i<nXsize * nYsize; i++) {
        ...
        if (/* 0<=nStart && */ nStart<=nFinish
            ...
            int nVerifyResult=verifySubTileFile(fp,
                                                nStart, nFinish,
                                                block_xx, block_yy,
                                                i / nYsize, i % nYsize
                                                );
            if (nVerifyResult<0) {
                ...
                if (0 == (0x0100 & nVerifyResult)) {
                    CPLDebug("Viewranger",
                             "raw data for tile %d, %d did not verify\n",
                             i / nYsize, i % nYsize);
                }
            }
        } else {
    ...
    for (int loopX=0; loopX < nRawXcount; loopX++) {
        for (int loopY=0; loopY < nRawYcount; loopY++) {
            size_t loop = static_cast<size_t>
...
)
{
    ...
    unsigned int nLen = static_cast<unsigned int>(finish-start);

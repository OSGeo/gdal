/******************************************************************************
 * $Id: VRHV.cpp,v 1.98 2021/04/28 15:35:59 werdna Exp $
 *
 * Project:  GDAL
...
CPL_CVSID("$Id: VRHV.cpp,v 1.98 2021/04/28 15:35:59 werdna Exp $")
...
char *VRHVDataset::VRHGetString( VSILFILE *fp, unsigned long long byteaddr )
{
    ...
    size_t ustring_length = static_cast<size_t>(string_length);
...
public:
    ...
    VRHRasterBand( VRHVDataset *, int , int);
...
CPLErr VRHRasterBand::IReadBlock( int nBlockXOff, int nBlockYOff,
                                  void * pImage )
{
    VRHVDataset *poGDS = static_cast<VRHVDataset *>(poDS);
    ...
    if ( poGDS->nMagic == vrh_magic ) {
        ...
        if ( poGDS->anColumnIndex[nBlockXOff] ) {
            ...
            signed short *panColumn = static_cast<short*>(pImage);
            for (i=0; i < nBlockYSize; i++) {
...
/* -------------------------------------------------------------------- */
    ...
    case vrh_magic:
        {
            // .VRH height file
            unsigned int vrh_header_offset;
            ...
            if (poDS->nPixelMetres < 1) {
                ...
                double dfPixelMetres = static_cast<double>(poDS->nPixelMetres);
    ...
    case vrv_magic:
        ...
        poDS->pszLongTitle = // poOpenInfo->pszFilename might be good
            VRHGetString(poDS->fp, 0x24);
    ...
    case vmc_magic:
        ...
        poDS->nScale = VRGetUInt( poDS->abyHeader, 0x20 );
        {
            ...
            // CPLDebug dosn't count as "using" a variable
            (void) l5;
            (void) dc1;
            (void) p;
        }
...
#define MAX_Y 1024 // 3267
    if  (poDS->nRasterXSize >MAX_X || poDS->nRasterYSize > MAX_Y )
    {
        if ( poDS->nMagic != vrh_magic ) {
             ...
             * and is not neccessary for any files yet found.
             */
        }
        CPLDebug("ViewrangerHV",
                 "Unsupported dimensions : %d x %d (max %d x %d)", 
...
/* -------------------------------------------------------------------- */
    VRHRasterBand *poBand =
      new VRHRasterBand( poDS, 1, 01);
                             ...
                             void *pimage)
{
    ...
    signed short *pnBottomPixel;
    pnBottomPixel = static_cast<signed short*>(pimage);
    // pnBottomPixel += static_cast<VRHVDataset *>(poDS)->nRasterYSize * tile_xx;

    signed short *pnOutPixel;
    pnOutPixel = pnBottomPixel + static_cast<VRHVDataset *>(poDS)->nRasterYSize - 1;
                             ...
                             void *pimage)
{
    ...
    unsigned char *pnBottomPixel;
    pnBottomPixel = static_cast<unsigned char*>(pimage);
    // pnBottomPixel += static_cast<VRHVDataset *>(poDS)->nRasterYSize * tile_xx;

    // char *pnOutPixel = pnBottomPixel + static_cast<VRHVDataset *>(poDS)->nRasterYSize - 1;

    unsigned int nCurrentData=static_cast<unsigned int>(VRReadChar(fp));
    int nBitsLeft=8;
    int nBytesRead=1;
    int nMaxPix=0;
    for (x=0; x<nBlockXSize; x++) {
        for (y=nBlockYSize-1 ; y>=0; y--) {
                             ...
                             void *pimage)
{
    ...
    int seekres;
    seekres = VSIFSeekL( fp, 0x24, SEEK_SET );
    if ( seekres ) {
        CPLError( CE_Failure, CPLE_AppDefined,
    ...
    char *pnBottomPixel;
    pnBottomPixel = static_cast<char*>(pimage);
    // pnBottomPixel += static_cast<VRHVDataset *>(poDS)->nRasterYSize * tile_xx;

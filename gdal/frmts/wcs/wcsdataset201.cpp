                                            ...
                                            CPL_UNUSED CPLString osBandList)
{
    ...
    tmp.Printf("&SUBSET=%s%%28%.15g,%.15g%%29", domain[0].c_str(), extent[0], extent[2]);
    request += tmp;
    tmp.Printf("&SUBSET=%s%%28%.15g,%.15g%%29", domain[1].c_str(), extent[1], extent[3]);
    request += tmp;
                                ...
                                char ***metadata)
{
    ...
    origin = Flist(Split(CPLGetXMLValue(point, NULL, ""), " ", axis_order_swap));
    ...
    if (subtype == "RectifiedGrid") {
        ...
        for (CPLXMLNode *node = grid->psChild; node != NULL; node = node->psNext) {
            ...
            offset.push_back(Flist(Split(CPLGetXMLValue(node, NULL, ""), " ", axis_order_swap)));
            if (i == 1) {
                break;
            }
            i++;
        ...
        /*
          me thinks that the order of the offsets should also be swapped??
        ...
        */
        ...
        for (CPLXMLNode *node = grid->psChild; node != NULL; node = node->psNext) {
            ...
            if (offset_node) {
                offset.push_back(Flist(Split(CPLGetXMLValue(offset_node, NULL, ""), " ", axis_order_swap)));
            } else {
    ...
    for (CPLXMLNode *field = record->psChild; field != NULL; field = field->psNext) {
        ...
        if (range.size() > 0) {
            ...
            if (range_index < range.size()) {
                ...
                fprintf(stderr, "%s %s, %s, %s\n", fname.c_str(), fname_test.c_str(), current_range.c_str(), (include ? "yes":"no"));
            }
        }

...
//	printf("Loading blocks %d %d %d %d\n", bx0, by0, bx1, by1);
	...
	if (request_count > 0) {
		CPLHTTPFetchMulti(download_requests, request_count);
	}

	for (int i = 0; i < request_count; ++i) {
//		printf("%d %p %d %s\n", download_requests[i].nStatus, download_requests[i].pabyData, download_requests[i].nDataLen, download_requests[i].pszURL);
...
//	printf("[%p] GDALWMSRasterBand::IRasterIO(x0: %d, y0: %d, sx: %d, sy: %d, bsx: %d, bsy: %d)\n", this, x0, y0, sx, sy, bsx, bsy);
...
//	printf("[%p] GDALWMSRasterBand::AddOverview(%f)\n", this, scale);
...
//	printf("[%p] GDALWMSRasterBand::ReadBlockFromFile(%d, %d, %s, %d, %p)\n", this, x, y, file_name, to_buffer_band, buffer);

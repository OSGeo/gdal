...
<Cache>                                                                    Enable local disk cache. Allows for offline operation. (optional, defaults to cache enabled)
...
<Type>file</Type>                                                          Cache type. Now supported only 'file' and 'none' type. In 'file' cache type files are stored in file system folders. (optional, defaults to 'file' if GDAL_DEFAULT_WMS_CACHE_TYPE configuration option is not specified)

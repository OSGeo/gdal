...
class CPL_DLL swq_parse_context {
public:
...
*/
int CPL_DLL swqparse( swq_parse_context *context );
int CPL_DLL swqlex( swq_expr_node **ppNode, swq_parse_context *context );
void CPL_DLL swqerror( swq_parse_context *context, const char *msg );

int CPL_DLL swq_identify_field( const char* table_name,
                                const char *token, swq_field_list *field_list,
                                swq_field_type *this_type, int *table_id );

CPLErr CPL_DLL swq_expr_compile( const char *where_clause,
                                 int field_count,
                                 char **field_list,
...
CPLErr CPL_DLL swq_expr_compile2( const char *where_clause,
...
** Evaluation related.
*/
int CPL_DLL swq_test_like( const char *input, const char *pattern );

swq_expr_node CPL_DLL *SWQGeneralEvaluator( swq_expr_node *, swq_expr_node **);
swq_field_type CPL_DLL SWQGeneralChecker( swq_expr_node *node, int bAllowMismatchTypeOnFieldComparison );
swq_expr_node CPL_DLL *SWQCastEvaluator( swq_expr_node *, swq_expr_node **);
swq_field_type CPL_DLL SWQCastChecker( swq_expr_node *node, int bAllowMismatchTypeOnFieldComparison );
const char CPL_DLL    *SWQFieldTypeToString( swq_field_type field_type );
...
class CPL_DLL swq_select_parse_options
{
public:
    swq_custom_func_registrar* poCustomFuncRegistrar;
...
CPLErr CPL_DLL swq_select_parse( swq_select *select_info,
                                 swq_field_list *field_list,
                                 int parse_flags );

const char CPL_DLL *swq_select_summarize( swq_select *select_info,
                                          int dest_column,
                                          const char *value );

int CPL_DLL swq_is_reserved_keyword(const char* pszStr);

char CPL_DLL *OGRHStoreGetValue(const char* pszHStore, const char* pszSearchedKey);

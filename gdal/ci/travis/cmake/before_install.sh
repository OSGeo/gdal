...
sudo chroot "$chroot" add-apt-repository -y ppa:miurahr/gta
sudo chroot "$chroot" add-apt-repository -y ppa:miurahr/librasterlite2
sudo chroot "$chroot" apt-get update
sudo chroot "$chroot" apt-get install -y postgis postgresql-9.5 postgresql-client-9.5 postgresql-9.5-postgis-2.2 postgresql-9.5-postgis-scripts libpq-dev
...
sudo chroot "$chroot" apt-get install -y librasterlite2-dev libgta-dev
sudo chroot "$chroot" apt-get install -y doxygen texlive-latex-base
sudo chroot "$chroot" apt-get install -y make cmake

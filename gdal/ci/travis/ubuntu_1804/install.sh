...
# Build proj
chroot "$chroot" sh -c "cd $PWD/proj && ./autogen.sh && CC='ccache gcc' CXX='ccache g++' CFLAGS='-DPROJ_RENAME_SYMBOLS' CXXFLAGS='-DPROJ_RENAME_SYMBOLS' ./configure --prefix=/usr/local || cat config.log"
chroot "$chroot" sh -c "cd $PWD/proj && CCACHE_CPP2=yes make -j3"
sudo chroot "$chroot" sh -c "cd $PWD/proj && make -j3 install && mv /usr/local/lib/libproj.so.13.1.1 /usr/local/lib/libinternalproj.so.13.1.1 && rm /usr/local/lib/libproj.so* && rm /usr/local/lib/libproj.a && rm /usr/local/lib/libproj.la && ln -s libinternalproj.so.13.1.1  /usr/local/lib/libinternalproj.so.13 && ln -s libinternalproj.so.13.1.1  /usr/local/lib/libinternalproj.so"

...
if grep -i warning docs_log.txt | grep -E -v '(russian|brazilian)'; then echo "Doxygen warnings found" && cat docs_log.txt && /bin/false; else echo "No Doxygen warnings found"; fi

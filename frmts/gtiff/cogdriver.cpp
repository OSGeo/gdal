...
{
  public:
    COGProxyDataset(const char *pszFilename, CSLConstList papszOptions,
                    std::unique_ptr<GDALDataset> poGTiffTmpDS)
        : m_osFilename(pszFilename), m_aosOptions(papszOptions),
          m_poGTiffTmpDS(std::move(poGTiffTmpDS))
    {
        eAccess = GA_Update;
                              ...
                              char **papszOptions)
{
    ...
    return std::make_unique<COGProxyDataset>(pszFilename, papszOptions,
                                             std::move(poGTiffTmpDS))
        .release();
}

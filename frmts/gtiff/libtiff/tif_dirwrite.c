                                 ...
                                 uint64_t *pdiroff)
{
    ...
    while (1)
    {
        /* First loop just determines "ndir" and
         * establishes the IFD file offset using TIFFLinkDirectory().
         * Second loop writes IFD entries to file.
         */
        ndir = 0;
        if (dir == NULL)
            tif->tif_dirdatasize_write = 0;
        if (isimage)
        {
         ...
         * themselves. */
        if (!(tif->tif_flags & TIFF_BIGTIFF))
            tif->tif_dirdatasize_write += 2 + ndir * 12 + 4;
        else
            tif->tif_dirdatasize_write += 8 + ndir * 20 + 8;
        ...
        if (isimage)
        {
            ...
            else if (tif->tif_dirdatasize_write > tif->tif_dirdatasize_read)
            {
                if (dir != NULL)
                {
        ...
        else
        {
             ...
             * the end of the file.
             */
            if (!((tif->tif_dirdatasize_read > 0) &&
                  (tif->tif_dirdatasize_write <= tif->tif_dirdatasize_read)))
            {
        ...
        if (!(tif->tif_flags & TIFF_BIGTIFF))
            ...
            dirsize = 8 + ndir * 20 + 8;
        /* Append IFD data stright after the IFD entries.
         * Data that does not fit into an IFD entry is written to the file in
         * the second pass of the while loop. That offset is stored in "dir". */
    ...
    {
        /* IFD is only checkpointed to file, thus set IFD size written to file.
         */
        tif->tif_dirdatasize_read = tif->tif_dirdatasize_write;
        tif->tif_dirdatasize_Nread = ndir;
    }
    return (1);
bad:
    if (dir != NULL)
                        ...
                        uint64_t uncompressed_threshold)
{
    ...
    {
        /* For a few select compression types, we assume that in the worst case
         * the compressed size will be 10 times the uncompressed size. This is
         * overly pessismistic ! */
                                     ...
                                     uint32_t typesize, uint32_t *ndir)
{
    ...
    {
        tif->tif_dirdatasize_write += datalength;
    }
    (*ndir)++;
}

...
void qh_check_maxout(qhT *qh) {
  ...
  trace1((qh, qh->ferr, 1024, "qh_check_maxout:  p%d(v%d) is qh.min_vertex %2.2g below facet f%d.  Point p%d for f%d is qh.max_outside %2.2g above f%d.  %d points are outside of not-good facets\n",
    qh_pointid(qh, minvertex->point), minvertex->id, qh->min_vertex, minfacet->id, qh_pointid(qh, maxpoint), maxpointfacet->id, qh->max_outside, maxfacet ? maxfacet->id : -1, notgood));
  if(!qh->ALLOWwide) {
    ...
    if (numgood == 0 && (goodhorizon == 0 || qh->GOODclosest)) {
      if (qh->GOODclosest) {
        if (qh->GOODclosest->visible)
          ...
          int ret = qh_inthresholds(qh, qh->GOODclosest->normal, &angle);
          QHULL_UNUSED(ret);
          if (angle < bestangle)

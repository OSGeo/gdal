...
echo "Checking for missing #include <cinttypes> statements..."
rm -f /tmp/missing_include.txt
while read -r i; do
   grep -e PRId64 -e PRIu64 $i >/dev/null && (grep "#include <cinttypes>" $i >/dev/null || echo $i) | grep -v cpl_json_header.h | grep -v json_inttypes.h | grep -v tif_config.h | tee -a /tmp/missing_include.txt;
done < /tmp/gdal_list_files.txt


if test -s /tmp/missing_include.txt; then
    echo "FAIL: missing #include <cinttypes> in above listed files"
    ret_code=1
else
    echo "OK."
fi

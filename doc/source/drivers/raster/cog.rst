...
that is it natively does not support random writing, but requires a source dataset
to be provided as input. During the writing process, if overviews are generated,
they will be generated first in a temporary dataset, requiring storage capacity
of roughly one third of the final file size. By default temporary files are
created in the same directory as the final file, if the target file system supports
random writing, and if the :config:`CPL_TMPDIR` configuration option is not set.

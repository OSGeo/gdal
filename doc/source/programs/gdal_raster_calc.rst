...
.. option:: --dialect muparser|builtin
    ...
    Specify with which expression engine the value of :option:`--dialect` must
    be interpreted. The default is ``muparser``.
...
.. example::
   :title: Per-dataset average of all bands o each dataset
   :id: simple-sum
...
.. example::
   :title: Compute the average (as a single band) of all bands of the 2 input datasets

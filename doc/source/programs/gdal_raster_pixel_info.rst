...
- through a GDAL vector dataset through the :option:`--coordinate-dataset` argument.
...
.. option:: --coordinate-dataset <COORDINATE-DATASET>

    .. versionadded:: 3.13

    GDAL-compatible vector dataset into which input coordinates are specified.
    The input coordinates are taken from the geometry of features. When this geometry
    is not a point, its centroid is taken into account. Attribute fields are
...
.. option:: -l, --layer, --input-layer <INPUT-LAYER>
    ...
    Input layer name of the dataset specified with :option:`--coordinate-dataset` to use.
...
.. option:: -p, --pos, --position <column,line> or <X,Y>
    ...
    Required, if :option:`--coordinate-dataset` is not specified and no content

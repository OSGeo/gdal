...
/************************************************************************/
/*                      ComputeInterBandCovariance()                    */
/************************************************************************/

/**
 * \brief Compute the covariance between 2 bands.
 *
 * This is a helper function used by GDALDataset::ComputeInterBandCovarianceMatrix(),
 * but which can also be used in a standalone way.
 *
 * @param poOtherBand Other band that must have the same dimension as this band.
 ...
 *                              Defaults to 1.
 * @param pdfThisBandMean Pointer to a value with the mean value of this band
 *                        when known, or nullptr if unknown.
 * @param pdfOtherBandMean Pointer to a value with the mean value of the other band
 *                        when known, or nullptr if unknown.
 * @param padfTempVector Pointer to a vector used for intermediate computation, or nullptr
 * @param pfnProgress a function to call to report progress, or NULL.
 * @param pProgressData application data to pass to the progress function.
 *
 * @return the covariance between the pixel values of the two bands, or NaN
 *         in case of error
 *
 * @since 3.13
 *
 * @see GDALDataset::ComputeInterBandCovarianceMatrix()
 */
double GDALRasterBand::ComputeInterBandCovariance(
    GDALRasterBand *poOtherBand, bool bApproxOK, int nDeltaDegreeOfFreedom,
    const double *pdfThisBandMean, const double *pdfOtherBandMean,
    std::vector<double> *padfTempVector, GDALProgressFunc pfnProgress,
    void *pProgressData)
{
    constexpr double ERROR_VALUE = std::numeric_limits<double>::quiet_NaN();

    if (nRasterXSize != poOtherBand->GetXSize() ||
        nRasterYSize != poOtherBand->GetYSize())
    {
        ReportError(CE_Failure, CPLE_AppDefined,
                    "ComputeInterBandCovariance(): other band not of the same "
                    "dimension as this band");
        return ERROR_VALUE;
    }

    if (bApproxOK && GetOverviewCount() > 0 &&
        poOtherBand->GetOverviewCount() > 0)
    {
        GDALRasterBand *poThisOvrBand =
            GetRasterSampleOverview(GDALSTAT_APPROX_NUMSAMPLES);
        GDALRasterBand *poOtherOvrBand =
            poOtherBand->GetRasterSampleOverview(GDALSTAT_APPROX_NUMSAMPLES);
        if (poThisOvrBand->GetXSize() == poOtherOvrBand->GetXSize() &&
            poThisOvrBand->GetYSize() == poOtherOvrBand->GetYSize())
        {
            return poThisOvrBand->ComputeInterBandCovariance(
                poOtherOvrBand, false,
                // As we subsample, we need to take into account the bias
                nDeltaDegreeOfFreedom == 0 ? 1 : nDeltaDegreeOfFreedom,
                pdfThisBandMean, pdfOtherBandMean, padfTempVector, pfnProgress,
                pProgressData);
        }
    }
    ...
    double dfThisMean = 0;
    if (pdfThisBandMean)
        dfThisMean = *pdfThisBandMean;
    else if (ComputeStatistics(bApproxOK, nullptr, nullptr, &dfThisMean,
                               nullptr, nullptr, nullptr) != CE_None)
    {
        return ERROR_VALUE;
    }

    double dfOtherMean = 0;
    if (pdfOtherBandMean)
        dfOtherMean = *pdfOtherBandMean;
    else if (poOtherBand->ComputeStatistics(bApproxOK, nullptr, nullptr,
                                            &dfOtherMean, nullptr, nullptr,
                                            nullptr) != CE_None)
    {
        return ERROR_VALUE;
    }

    int bThisHasNoData = false;
    double dfThisNoData = GetNoDataValue(&bThisHasNoData);
    if (!bThisHasNoData)
        dfThisNoData = std::numeric_limits<double>::quiet_NaN();

    int bOtherHasNoData = false;
    double dfOtherNoData = poOtherBand->GetNoDataValue(&bOtherHasNoData);
    if (!bOtherHasNoData)
        dfOtherNoData = std::numeric_limits<double>::quiet_NaN();

    GDALRasterBand *poThisMaskBand = nullptr;
    std::vector<GByte> abyThisMaskBand;
    if (!bThisHasNoData)
    {
        if (GetMaskFlags() != GMF_ALL_VALID &&
            GetColorInterpretation() != GCI_AlphaBand)
        {
            poThisMaskBand = GetMaskBand();
            try
            {
                abyThisMaskBand.resize(nRasterXSize);
            }
            catch (const std::exception &)
            {
                ReportError(CE_Failure, CPLE_OutOfMemory,
    ...
    GDALRasterBand *poOtherMaskBand = nullptr;
    std::vector<GByte> abyOtherMaskBand;
    if (!bOtherHasNoData)
    {
        if (poOtherBand->GetMaskFlags() != GMF_ALL_VALID &&
            poOtherBand->GetColorInterpretation() != GCI_AlphaBand)
        {
            poOtherMaskBand = poOtherBand->GetMaskBand();
            if (poOtherMaskBand == poThisMaskBand)
            {
                // When called on bands of the same dataset, this situation
                // should be common.
                poOtherMaskBand = nullptr;
            }
            else
            {
                try
                {
                    abyOtherMaskBand.resize(nRasterXSize);
                }
                catch (const std::exception &)
                {
                    ReportError(
    ...
    std::vector<double> adfTempVector;
    if (!padfTempVector)
        padfTempVector = &adfTempVector;

    // One for padfThisValues and another one for padfOtherValues
    constexpr int VECTOR_COUNT = 2;
    if constexpr (sizeof(size_t) < sizeof(uint64_t))
    {
        if (static_cast<size_t>(nRasterXSize) >
            std::numeric_limits<size_t>::max() / VECTOR_COUNT)
        {
            ReportError(CE_Failure, CPLE_OutOfMemory,
                        "Not enough memory for intermediate computations");
            return ERROR_VALUE;
    ...
    try
    {
        if (padfTempVector->size() <
            static_cast<size_t>(nRasterXSize) * VECTOR_COUNT)
            padfTempVector->resize(static_cast<size_t>(nRasterXSize) *
                                   VECTOR_COUNT);
    }
    catch (const std::exception &)
    {
    ...
    double *const padfThisValues = padfTempVector->data();
    double *const padfOtherValues =
        poOtherBand != this ? padfThisValues + nRasterXSize : padfThisValues;
    ...
    const bool bIsIntegerCaseNoMask =
        ...
        GDALDataTypeIsInteger(poOtherBand->GetRasterDataType()) &&
        !bThisHasNoData && !bOtherHasNoData && !poThisMaskBand &&
        !poOtherMaskBand;

    for (int y = 0; y < nRasterYSize; ++y)
    {
        // We could also proceed by blocks instead of scanlines
        if (RasterIO(GF_Read, 0, y, nRasterXSize, 1, padfThisValues,
                     nRasterXSize, 1, GDT_Float64, 0, 0, nullptr) != CE_None ||
            (poOtherBand != this &&
             poOtherBand->RasterIO(GF_Read, 0, y, nRasterXSize, 1,
                                   padfOtherValues, nRasterXSize, 1,
                                   GDT_Float64, 0, 0, nullptr) != CE_None) ||
            (poThisMaskBand &&
             poThisMaskBand->RasterIO(GF_Read, 0, y, nRasterXSize, 1,
                                      abyThisMaskBand.data(), nRasterXSize, 1,
                                      GDT_Byte, 0, 0, nullptr) != CE_None) ||
            (poOtherMaskBand &&
             poOtherMaskBand->RasterIO(GF_Read, 0, y, nRasterXSize, 1,
                                       abyOtherMaskBand.data(), nRasterXSize, 1,
                                       GDT_Byte, 0, 0, nullptr) != CE_None))
        {
            return ERROR_VALUE;
        }
        ...
        if (bIsIntegerCaseNoMask)
        {
            ...
            {
                const double a = padfThisValues[x];
                const double b = padfOtherValues[x];
                dfAcc += (a - dfThisMean) * (b - dfOtherMean);
            }
            nSampleCount += nRasterXSize;
        }
        else
        {
            // Generic code path
            for (int x = 0; x < nRasterXSize; ++x)
            {
                const double a = padfThisValues[x];
                const double b = padfOtherValues[x];
                if (!std::isnan(a) && a != dfThisNoData && !std::isnan(b) &&
                    b != dfOtherNoData &&
                    (abyThisMaskBand.empty() || abyThisMaskBand[x] != 0) &&
                    (abyOtherMaskBand.empty() || abyOtherMaskBand[x] != 0))
                {
                    nSampleCount += 1;
                    dfAcc += (a - dfThisMean) * (b - dfOtherMean);
                }
        ...
        if (pfnProgress &&
            !pfnProgress(static_cast<double>(y + 1) / nRasterYSize, "",
                         pProgressData))
        {
            ReportError(CE_Failure, CPLE_UserInterrupt, "User terminated");
            return ERROR_VALUE;
    ...
    const double dfCovariance =
        (nDeltaDegreeOfFreedom < 0 ||
         nSampleCount <= static_cast<uint64_t>(nDeltaDegreeOfFreedom))
            ? std::numeric_limits<double>::quiet_NaN()
            : dfAcc / (nSampleCount - nDeltaDegreeOfFreedom);

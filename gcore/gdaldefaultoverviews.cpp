    ...
    GDALDataset *poFullResDS, const std::vector<GDALDataset *> &apoSrcOvrDS)
{
    ...
    double adfThisGT[6] = {0};
    const bool bThisHasGT = poFullResDS->GetGeoTransform(adfThisGT) == CE_None;
    for (auto *poSrcOvrDS : apoSrcOvrDS)
    {
        ...
        if (bThisHasGT)
        {
            ...
            const bool bOvrHasGT =
                poSrcOvrDS->GetGeoTransform(adfOvrGT) == CE_None;
            ...
            if (bOvrHasGT &&
                !(std::fabs(adfThisGT[0] - adfOvrGT[0]) <=
                      0.5 * std::fabs(adfOvrGT[1]) &&
                  std::fabs(adfThisGT[1] - adfOvrGT[1] / dfOvrXRatio) <=
                      0.1 * std::fabs(adfOvrGT[1]) &&
                  std::fabs(adfThisGT[2] - adfOvrGT[2] / dfOvrYRatio) <=
                      0.1 * std::fabs(adfOvrGT[2]) &&
                  std::fabs(adfThisGT[3] - adfOvrGT[3]) <=
                      0.5 * std::fabs(adfOvrGT[5]) &&
                  std::fabs(adfThisGT[4] - adfOvrGT[4] / dfOvrXRatio) <=
                      0.1 * std::fabs(adfOvrGT[4]) &&
                  std::fabs(adfThisGT[5] - adfOvrGT[5] / dfOvrYRatio) <=
                      0.1 * std::fabs(adfOvrGT[5])))
            {
                CPLError(
                    CE_Failure, CPLE_AppDefined,
    ...
    [[maybe_unused]] CSLConstList papszOptions)
{
    ...
    if (!anOverviewSizes.empty())
    {
        ...
        CPLErr eErr =
            GTIFFBuildOverviewsEx(osOvrFilename, nBands, apoBands.data(),
                                  static_cast<int>(apoSrcOvrDS.size()), nullptr,
                                  anOverviewSizes.data(), "NONE", nullptr,

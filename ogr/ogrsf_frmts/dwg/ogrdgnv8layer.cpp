...
OGRDGNV8Layer::OGRDGNV8Layer(OGRDGNV8DataSource *poDS, OdDgModelPtr pModel)
    : m_poDS(poDS), m_poFeatureDefn(nullptr), m_pModel(pModel),
      ...
      m_nIdxInPendingFeatures(0)
{
    ...
    CPLDebug("DGNV8", "%s is %dd", pszName, pModel->getModelIs3dFlag() ? 3 : 2);
...
{
    for (size_t i = m_nIdxInPendingFeatures; i < m_aoPendingFeatures.size();
         i++)
    {
...
OGRDGNV8Layer::CollectSubElements(OdDgElementIteratorPtr iterator, int level)
{
...
static OdDg::TextJustification GetAnchorPositionFromOGR(int value)
{
    ...
    {
        case 1:
            return OdDg::kLeftDescender;
        case 2:
            return OdDg::kCenterDescender;
        case 3:
            return OdDg::kRightDescender;
        case 4:
            return OdDg::kLeftCenter;
        case 5:
            return OdDg::kCenterCenter;
        case 6:
            return OdDg::kRightCenter;
        case 7:
            return OdDg::kLeftTop;
        case 8:
            return OdDg::kCenterTop;
        case 9:
            return OdDg::kRightTop;
        case 10:
            return OdDg::kLeftBottom;
        case 11:
            return OdDg::kCenterBottom;
        case 12:
            return OdDg::kRightBottom;
        default:
            return OdDg::kLeftTop;
    }
}
...
static void ProcessText(OGRFeature *poFeature, const CPLString &osColor,
                        TextPtr text)
{
    ...
    CPLString osStyle;
    osStyle.Printf("LABEL(t:\"%s\"", EscapeDoubleQuote(ToUTF8(oText)).c_str());
    osStyle += osColor;
    osStyle += CPLSPrintf(",s:%fg", text->getHeightMultiplier());

    // Gets Font name
    OdDgFontTablePtr pFontTable = text->database()->getFontTable();
    OdDgFontTableRecordPtr pFont = pFontTable->getFont(text->getFontEntryId());
    if (!pFont.isNull())
    {
        osStyle += CPLSPrintf(
            ",f:\"%s\"", EscapeDoubleQuote(ToUTF8(pFont->getName())).c_str());
    }
    else
    {
        osStyle += CPLSPrintf(",f:MstnFont%u", text->getFontEntryId());
    }
    ...
    const double dfRotation = ProcessTextTraits<TextPtr>::getRotation(text);
    if (dfRotation != 0.0)
        osStyle += CPLSPrintf(
            ",a:%d", static_cast<int>(floor(dfRotation * RAD_TO_DEG + 0.5)));
                               ...
                               const CPLString &osStyle)
{
    ...
    if (linkages.length() >= 1)
    {
        ...
        if (!fillColor.isNull())
        {
            ...
            if (OdDgColorTable::isCorrectIndex(element->database(),
                                               uFillColorIdx))
            {
                         ...
                         CurveElementPtr curveElement)
{
    ...
    typedef
        typename ProcessCurveTraits<CurveElementPtr>::BSplineType BSplineType;
    typedef
        typename ProcessCurveTraits<CurveElementPtr>::EllipseType EllipseType;
    ...
    {
        bIsCircular =
            AlmostEqual(ellipse->getPrimaryAxis(), ellipse->getSecondaryAxis());
    }
    else if (!arc.isNull())
    {
        bIsCircular =
            AlmostEqual(arc->getPrimaryAxis(), arc->getSecondaryAxis());
    }
    ...
    else
    {
        ...
        {
            nPoints = std::max(2, static_cast<int>(arc->getSweepAngle() *
    ...
    for (int i = 0; i < nPoints; i++)
    {
        PointType point;
        double dfParam =
            dfStartParam + i * (dfEndParam - dfStartParam) / (nPoints - 1);
...
static bool
IsContiguous(const std::vector<tPairFeatureHoleFlag> &oVectorSubElts,
             bool &bHasCurves, bool &bIsClosed)
{
    bHasCurves = false;
    bIsClosed = false;
    ...
    for (size_t i = 0; i < oVectorSubElts.size(); i++)
    {
        ...
        if (poGeom != nullptr)
        {
            ...
            if (bIsContiguous &&
                (eType == wkbCircularString || eType == wkbLineString))
            {
                    ...
                    if (bLastPointValid)
                    {
                        if (!AlmostEqual(oStartPoint.getX(),
                            ...
                            !AlmostEqual(oStartPoint.getY(),
                                         oLastPoint.getY()) ||
                            !AlmostEqual(oStartPoint.getZ(), oLastPoint.getZ()))
                        {
                            bIsContiguous = false;
                            break;
                        }
...
std::vector<tPairFeatureHoleFlag>
OGRDGNV8Layer::ProcessElement(OdDgGraphicsElementPtr element, int level)
{
    std::vector<tPairFeatureHoleFlag> oVector;
#ifdef DEBUG_VERBOSE
    CPLString osIndent;
    ...
    {
        poFeature->SetField("ColorIndex", static_cast<int>(uColorIndex));
    ...
    if (linkages.size() > 0)
    {
        for (unsigned i = 0; i < linkages.size(); ++i)
        {
            ...
            switch (primaryId)
            {
                case OdDgAttributeLinkage::kFRAMME:  // DB Linkage - FRAMME tag
                                                     // data signature
                case OdDgAttributeLinkage::kBSI:    // DB Linkage - secondary id
                                                    // link (BSI radix 50)
                case OdDgAttributeLinkage::kXBASE:  // DB Linkage - XBase
                                                    // (DBase)
                ...
                case OdDgAttributeLinkage::kRIS:       // DB Linkage - RIS
                {
                    ...
                    if (!dbLinkage.isNull())
                    {
                        ...
                        theNewObject.Add("tableId",
                                         int(dbLinkage->getTableEntityId()));
                ...
                case 0x1995:  // 0x1995 (6549) IPCC/Portugal
                {
                    ...
                    theNewObject.Add("subdomain",
                                     CPLSPrintf("0x%02x", pData[4]));
                    theNewObject.Add("family", CPLSPrintf("0x%02x", pData[7]));
                    theNewObject.Add("object", CPLSPrintf("0x%02x", pData[6]));
                    theNewObject.Add("key",
                                     CPLSPrintf("%02x%02x%02x%02x", pData[5],
                                                pData[4], pData[7], pData[6]));
                    theNewObject.Add("type", "IPCC/Portugal");
                }
                break;
                case OdDgAttributeLinkage::kString:  // 0x56d2 (22226):
                {
                    OdDgStringLinkagePtr pStrLinkage =
                        OdDgStringLinkage::cast(pLinkage);
                    if (!pStrLinkage.isNull())
                    {
                        theNewObject.Add(
                            "string", ToUTF8(pStrLinkage->getString()).c_str());
                ...
                default:
                {
                    ...
                    {
                        rawWords.Add(
                            CPLSPrintf("0x%02x%02x", pData[k + 1], pData[k]));
                    }
        ...
        if (!iterator.isNull())
        {
            ...
            if (nCountMain == 1 && bHasHole)
            {
                ...
                if (bHasCurve)
                    ...
                    poCP = new OGRPolygon();
                poCP->addRing(
                    poExterior->toCurvePolygon()->getExteriorRingCurve());
                for (size_t i = 0; i < oVector.size(); i++)
                {
                    OGRFeature *poFeat = oVector[i].first;
                    OGRGeometry *poGeom = poFeat->GetGeometryRef();
    ...
    {
        OdDgCurveElement2dPtr curveElement = OdDgCurveElement2d::cast(element);
    ...
    {
        OdDgCurveElement3dPtr curveElement = OdDgCurveElement3d::cast(element);
        CPLAssert(!curveElement.isNull());
    ...
    else if (EQUAL(pszEntityClassName, "OdDgComplexString"))
    {
        ...
        if (!iterator.isNull())
        {
            ...
            bool bIsContiguous =
                IsContiguous(oVectorSubElts, bHasCurves, bIsClosed);
    ...
    else if (EQUAL(pszEntityClassName, "OdDgComplexShape"))
    {
        ...
        if (!iterator.isNull())
        {
            ...
            if (bIsContiguous && bIsClosed)
            {
                ...
                poCP->addRingDirectly((bHasCurves) ? poCC->toCurve()
                                                   : poLR->toCurve());
    ...
    {
        OdDgSharedCellReferencePtr ref = OdDgSharedCellReference::cast(element);
        CPLAssert(!ref.isNull());
        OdGePoint3d point = ref->getOrigin();
        poFeature->SetField("Text", ToUTF8(ref->getDefinitionName()).c_str());
        poFeature->SetGeometryDirectly(new OGRPoint(point.x, point.y, point.z));
    }
    else
    {
...
OGRErr OGRDGNV8Layer::DeleteFeature(GIntBig nFID)
{
    ...
    OdDgGraphicsElementPtr element = GetFeatureInternal(nFID, OdDg::kForWrite);
    if (element.isNull())
        return OGRERR_FAILURE;
    try
    {
...
bool OGRDGNV8Layer::TestCapability(const char *pszCap) const
...

{
    ...
    try
    {
        OdDgGraphicsElementPtr element =
            CreateGraphicsElement(poFeature, poFeature->GetGeometryRef());
        if (element.isNull())
            return OGRERR_FAILURE;
...
OdDgGraphicsElementPtr OGRDGNV8Layer::TranslateLabel(OGRFeature *poFeature,
                                                     OGRPoint *poPoint)

{
    ...
    if (poLabel != nullptr)
    {
        ...
        if (!bDefault && pszFontName != nullptr)
        {
            ...
            OdDgElementId idFont =
                pFontTable->getAt(OGRDGNV8DataSource::FromUTF8(pszFontName));
            if (!idFont.isNull())
            {
...
int OGRDGNV8Layer::GetColorFromString(const char *pszColor)
{
    ...
    const int nCount = sscanf(pszColor, "#%2x%2x%2x", &nRed, &nGreen, &nBlue);
    if (nCount == 3)
    {
                                      ...
                                      OdDgComplexCurvePtr complexCurve)
{
    for (int i = 0; i + 2 < poCS->getNumPoints(); i += 2)
    {
        ...
        if (OGRGeometryFactory::GetCurveParameters(
                poCS->getX(i), poCS->getY(i), poCS->getX(i + 1),
                poCS->getY(i + 1), poCS->getX(i + 2), poCS->getY(i + 2), R, cx,
                cy, alpha0, alpha1, alpha2))
        {
                         ...
                         double &R)
{
    ...
    else if (poCS->getNumPoints() == 5 && poCS->get_IsClosed())
    {
        ...
        double alpha2_2 = 0.0;
        if (OGRGeometryFactory::GetCurveParameters(
                poCS->getX(0), poCS->getY(0), poCS->getX(1), poCS->getY(1),
                poCS->getX(2), poCS->getY(2), R_1, cx_1, cy_1, alpha0_1,
                alpha1_1, alpha2_1) &&
            OGRGeometryFactory::GetCurveParameters(
                poCS->getX(2), poCS->getY(2), poCS->getX(3), poCS->getY(3),
                poCS->getX(4), poCS->getY(4), R_2, cx_2, cy_2, alpha0_2,
                alpha1_2, alpha2_2) &&
            AlmostEqual(R_1, R_2) && AlmostEqual(cx_1, cx_2) &&
...
OdDgGraphicsElementPtr
OGRDGNV8Layer::CreateGraphicsElement(OGRFeature *poFeature, OGRGeometry *poGeom)

{
    ...
    else if (eFType == wkbCircularString)
    {
        ...
        else if (poCS->getNumPoints() == 3 && !OGR_GT_HasZ(eType))
        {
            ...
            if (OGRGeometryFactory::GetCurveParameters(
                    poCS->getX(0), poCS->getY(0), poCS->getX(1), poCS->getY(1),
                    poCS->getX(2), poCS->getY(2), R, cx, cy, alpha0, alpha1,
                    alpha2))
            {
                OdDgArc2dPtr arc = OdDgArc2d::createObject();
                element = arc;
    ...
    else if (eFType == wkbCurvePolygon || eFType == wkbPolygon)
    {
        ...
        else
        {
            if (OGR_GT_HasZ(eType))
            {
                ...
                element = pCell;
                for (int iRing = -1; iRing < poPoly->getNumInteriorRings();
                     iRing++)
                {
                    OGRCurve *poCurve =
                        (iRing < 0) ? poPoly->getExteriorRingCurve()
                                    : poPoly->getInteriorRingCurve(iRing);

                    OdDgGraphicsElementPtr shape =
                        CreateShape(poFeature, poCurve, iRing >= 0);

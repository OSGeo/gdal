...
static ElementPtr OGRLIBKMLParse(const std::string &oKml, std::string *posError)
{
    try
    {
        // To allow reading files using an explicit namespace prefix like
        // <kml:kml> we need to use ParseNS (see #6981). But if we use ParseNS,
        // we have issues reading gx: elements. So use ParseNS only when we have
        // errors with Parse. This is not completely satisfactory...
...
OGRLIBKMLDataSource::OGRLIBKMLDataSource(KmlFactory *poKmlFactory)
    : papoLayers(nullptr), nLayers(0), nAllocated(0), bUpdate(false),
      bUpdated(false), m_papszOptions(nullptr), m_isKml(false),
...
static void OGRLIBKMLPreProcessInput(std::string &oKml)
{
    ...
    // Some non conformant file may contain
    // MultiPolygon/MultiLineString/MultiPoint See
    // https://github.com/OSGeo/gdal/issues/4031 Replace them by MultiGeometry.
    nPos = 0;
    while (true)
    {
...
{
    std::string oKmlFilename = GetDescription();
    ...
    if (m_poKmlDSContainer && m_poKmlDSContainer->IsA(kmldom::Type_Document))
    {
        ...
        for (int iLayer = 0; iLayer < nLayers; iLayer++)
        {
            ...
            if ((poKmlSchema = papoLayers[iLayer]->GetKmlSchema()))
            {
                ...
                for (size_t iKmlSchema = 0; iKmlSchema < nKmlSchemas;
                     iKmlSchema++)
                {
                    poKmlSchema2 =
                                      ...
                                      char **papszOptions = nullptr)
{
    ...
    const bool bWithAtom = pszAuthorName != nullptr ||
                           pszAuthorURI != nullptr ||
                           pszAuthorEmail != nullptr || pszLink != nullptr;
...
bool OGRLIBKMLDataSource::WriteKmz()
{
    ...
    if (!VSISupportsRandomWrite(GetDescription(), false) ||
    ...
    if (!hZIP)
    {
        CPLError(CE_Failure, CPLE_NoWriteAccess, "Error creating %s: %s",
                 GetDescription(), VSIGetLastErrorMsg());
        return false;
    }
    ...
    if (CPLTestBool(pszUseDocKml) && (m_poKmlDocKml || m_poKmlUpdate))
    {
        ...
        if (!m_poKmlDocKmlRoot)
        {
            ...
            ParseDocumentOptions(std::move(kml), AsDocument(m_poKmlDocKml));
        }
    ...
    for (int iLayer = 0; iLayer < nLayers && !m_poKmlUpdate; iLayer++)
    {
        ...
        if (poKmlContainer->IsA(kmldom::Type_Document))
        {
            ...
            papoLayers[iLayer]->Finalize(std::move(poKmlDocument));
        }
        ...
        if (CPLCreateFileInZip(hZIP, pszLayerFileName, nullptr) != CE_None ||
            CPLWriteFileInZip(hZIP, oKmlOut.data(),
                              static_cast<int>(oKmlOut.size())) != CE_None)
            CPLError(CE_Failure, CPLE_FileIO, "ERROR adding %s to %s",
                     papoLayers[iLayer]->GetFileName(), GetDescription());
        CPLCloseFileInZip(hZIP);
    }
    ...
    if (m_poKmlStyleKml)
    {
                              ...
                              static_cast<int>(oKmlOut.size())) != CE_None)
        {
            bRet = false;
            CPLError(CE_Failure, CPLE_FileIO, "ERROR adding %s to %s",
                     "style/style.kml", GetDescription());
        }
        CPLCloseFileInZip(hZIP);
    }
    ...
    if (!osTmpFilename.empty())
    {
        if (bRet)
        {
            bRet = CPLCopyFile(GetDescription(), osTmpFilename.c_str()) == 0;
            if (!bRet)
                CPLError(CE_Failure, CPLE_FileIO,
                         "Cannot copy temporary file to %s", GetDescription());
        }
    ...
    if (CPLTestBool(pszUseDocKml) && (m_poKmlDocKml || m_poKmlUpdate))
    {
        ...
        if (fp == nullptr)
        {
            CPLError(CE_Failure, CPLE_FileIO, "Error writing %s to %s: %s",
                     "doc.kml", GetDescription(), VSIGetLastErrorMsg());
            return false;
        }
    ...
    bool bNew, bool bUpdateIn, int nGuess)
{
    ...
    osUniqueLayername.toupper();
    m_oMapLayers[std::move(osUniqueLayername)] = poOgrLayer;
...
int OGRLIBKMLDataSource::ParseLayers(ContainerPtr poKmlContainer, bool bRecurse)
{
        ...
        if (poKmlFeat->IsA(kmldom::Type_Container))
        {
            if (bRecurse)
            {
                ...
                if (poKmlFeat->has_name())
                {
                    ...
                    while (nSize > 0 && (oKmlFeatName[nSize - 1] == ' ' ||
                ...
                ParseLayers(AsContainer(poKmlFeat), true);
            }
        }
        else
        {
...
int OGRLIBKMLDataSource::ParseIntoStyleTable(std::string *poKmlStyleKml,
                                             const char *pszMyStylePath)
{
    ...
    if (!(poKmlContainer =
              GetContainerFromRoot(m_poKmlFactory, std::move(poKmlRoot))))
    {
        return false;
    }
...
int OGRLIBKMLDataSource::OpenKml(const char *pszFilename, int bUpdateIn)
{
    ...
    if (fp == nullptr)
    {
        CPLError(CE_Failure, CPLE_OpenFailed, "Cannot open %s", pszFilename);
        return FALSE;
    }
    int nRead = 0;
    while ((nRead = static_cast<int>(
                VSIFReadL(&osBuffer[0], 1, osBuffer.size(), fp))) != 0)
    {
        try
        {
        ...
        {
            CPLDebug("LIBKML", "libstdc++ exception during ingestion: %s",
                     ex.what());
            VSIFCloseL(fp);
            return FALSE;
        }
    ...
    if (!m_poKmlDSKml)
    {
        CPLError(CE_Failure, CPLE_OpenFailed, "ERROR parsing kml %s :%s",
    ...
    if (!(m_poKmlDSContainer =
              GetContainerFromRoot(m_poKmlFactory, m_poKmlDSKml)))
    {
        CPLError(CE_Failure, CPLE_OpenFailed, "ERROR parsing kml %s :%s %s",
    ...
    /***** parse for layers *****/
    int nPlacemarks = ParseLayers(m_poKmlDSContainer, false);
    ...
    if (nPlacemarks)
    {
        ...
        AddLayer(layername_default.c_str(), wkbUnknown, nullptr, this,
                 m_poKmlDSKml, m_poKmlDSContainer, pszFilename, FALSE,
                 bUpdateIn, 1);
    }

    ParseLayers(m_poKmlDSContainer, true);
...
int OGRLIBKMLDataSource::OpenKmz(const char *pszFilename, int bUpdateIn)
{
    ...
    if (!poKmlKmzfile)
    {
        CPLError(CE_Failure, CPLE_OpenFailed, "%s is not a valid kmz file",
                 pszFilename);
        return FALSE;
    }
    ...
    if (!poKmlDocKmlRoot)
    {
        CPLError(CE_Failure, CPLE_OpenFailed,
                 "ERROR parsing kml layer %s from %s :%s", oKmlKmlPath.c_str(),
                 pszFilename, oKmlErrors.c_str());
    ...
    if (!(poKmlContainer =
              GetContainerFromRoot(m_poKmlFactory, poKmlDocKmlRoot)))
    {
        CPLError(CE_Failure, CPLE_OpenFailed, "ERROR parsing %s from %s :%s",
                 oKmlKmlPath.c_str(), pszFilename,
                 "kml contains no Containers");
    ...
    else
    {
        ...
        /***** parse for layers *****/
        const int nPlacemarks = ParseLayers(poKmlContainer, false);
...
int OGRLIBKMLDataSource::OpenDir(const char *pszFilename, int bUpdateIn)
{
    ...
    for (int iFile = 0; iFile < nFiles; iFile++)
    {
        ...
        int nRead = 0;
        while ((nRead = static_cast<int>(VSIFReadL(szBuffer, 1, 1024, fp))) !=
               0)
        {
            try
            {
        ...
        // Root document generated by ourselves. Contains nothing of interest
        if (poKmlContainer->has_id() && poKmlContainer->get_id() == "root_doc")
        {
            continue;
        }
...
int OGRLIBKMLDataSource::Open(const char *pszFilename, bool bUpdateIn)
{
    ...
    if (nRead == 1024 && szBuffer[0] == 0x50 && szBuffer[1] == 0x4B &&
    ...
    if (poKmlDocument)
    {
            ...
            pszAuthorEmail != nullptr)
        {
            ...
            if (pszAuthorEmail != nullptr)
            {
                ...
                if (pszArobase != nullptr &&
                    strchr(pszArobase + 1, '.') != nullptr)
                {
        ...
        createkmlliststyle(m_poKmlFactory, pszDocumentId, poKmlDocument,
                           poKmlDocument, osListStyleType, osListStyleIconHref);
    }
    ...
    if (poKml)
    {
        if (m_poKmlUpdate)
        {
            NetworkLinkControlPtr nlc =
                m_poKmlFactory->CreateNetworkLinkControl();
        ...
        if (pszNLCMinRefreshPeriod != nullptr ||
            pszNLCMaxSessionLength != nullptr || pszNLCCookie != nullptr ||
            pszNLCMessage != nullptr || pszNLCLinkName != nullptr ||
            pszNLCLinkDescription != nullptr || pszNLCLinkSnippet != nullptr ||
            pszNLCExpires != nullptr)
        {
            ...
            if (pszNLCLinkSnippet != nullptr)
            {
                ...
                nlc->set_linksnippet(std::move(linksnippet));
            }
    ...
    OGRwkbGeometryType eGType, CSLConstList papszOptions)
{
    ...
    if (!m_poKmlUpdate)
    {
        ...
        if (CPLTestBool(pszUseDocKml) && m_poKmlDocKml)
        {
            ...
            poKmlNetLink->set_name(
                CSLFetchNameValueDef(papszOptions, "NAME", pszLayerName));
                                  ...
                                  CSLConstList papszOptions)
{
    ...
    {
        poOgrLayer =
            CreateLayerKml(pszLayerName, poOgrSRS, eGType, papszOptions);
    }
    else if (IsKmz() || IsDir())
    {
        poOgrLayer =
            CreateLayerKmz(pszLayerName, poOgrSRS, eGType, papszOptions);
    }
    ...
    if (poOgrLayer != nullptr && pszLookatLongitude != nullptr &&
        pszLookatLatitude != nullptr && pszLookatRange != nullptr)
    {
        poOgrLayer->SetLookAt(pszLookatLongitude, pszLookatLatitude,
                              pszLookatAltitude, pszLookatHeading,
                              pszLookatTilt, pszLookatRange,
    ...
    else
    {
        ...
        if (poOgrLayer != nullptr && pszCameraLongitude != nullptr &&
            pszCameraLatitude != nullptr && pszCameraAltitude != nullptr &&
            pszCameraAltitudeMode != nullptr)
        {
            poOgrLayer->SetCamera(pszCameraLongitude, pszCameraLatitude,
                                  pszCameraAltitude, pszCameraHeading,
                                  pszCameraTilt, pszCameraRoll,
    ...
    {
        poOgrLayer->SetWriteRegion(
            CPLAtof(pszRegionMinLodPixels), CPLAtof(pszRegionMaxLodPixels),
            CPLAtof(pszRegionMinFadeExtent), CPLAtof(pszRegionMaxFadeExtent));
    ...
    const char *pszSODescription =
        CSLFetchNameValue(papszOptions, "SO_DESCRIPTION");
    ...
    const char *pszSOOverlayXUnits =
        CSLFetchNameValue(papszOptions, "SO_OVERLAY_XUNITS");
    const char *pszSOOverlayYUnits =
        CSLFetchNameValue(papszOptions, "SO_OVERLAY_YUNITS");
    ...
    const char *pszSOScreenXUnits =
        CSLFetchNameValue(papszOptions, "SO_SCREEN_XUNITS");
    const char *pszSOScreenYUnits =
        CSLFetchNameValue(papszOptions, "SO_SCREEN_YUNITS");
    ...
    const char *pszSOSizeXUnits =
        CSLFetchNameValue(papszOptions, "SO_SIZE_XUNITS");
    const char *pszSOSizeYUnits =
        CSLFetchNameValue(papszOptions, "SO_SIZE_YUNITS");
    if (poOgrLayer != nullptr && pszSOHref != nullptr)
    {
        poOgrLayer->SetScreenOverlay(
            pszSOHref, pszSOName, pszSODescription, pszSOOverlayX,
            pszSOOverlayY, pszSOOverlayXUnits, pszSOOverlayYUnits, pszSOScreenX,
            pszSOScreenY, pszSOScreenXUnits, pszSOScreenYUnits, pszSOSizeX,
            pszSOSizeY, pszSOSizeXUnits, pszSOSizeYUnits);
    }

    const char *pszListStyleType =
        CSLFetchNameValue(papszOptions, "LISTSTYLE_TYPE");
    const char *pszListStyleIconHref =
        CSLFetchNameValue(papszOptions, "LISTSTYLE_ICON_HREF");
...
void OGRLIBKMLDataSource::SetStyleTable2Kml(OGRStyleTable *poStyleTable)
{
    ...
    DocumentPtr poKmlDocument = AsDocument(m_poKmlDSContainer);
    int nKmlStyles =
        static_cast<int>(poKmlDocument->get_styleselector_array_size());

...
TEST_F(test_gdal_minmax_element, float32)
{
    ...
    {
        T nodata = 2.0f;
        std::vector<T> v{cpl::CPLNumericLimits<T>::quiet_NaN(),
                         cpl::CPLNumericLimits<T>::quiet_NaN(), nodata, max_v,
                         min_v};
        auto idx_min = gdal::min_element(v.data(), v.size(), eDT, true, nodata);
        EXPECT_EQ(v[idx_min], min_v);
        auto idx_max = gdal::max_element(v.data(), v.size(), eDT, true, nodata);
    ...
    {
        T nodata = cpl::CPLNumericLimits<T>::quiet_NaN();
        std::vector<T> v{cpl::CPLNumericLimits<T>::quiet_NaN(),
                         cpl::CPLNumericLimits<T>::quiet_NaN(), nodata, max_v,
                         min_v};
        auto idx_min = gdal::min_element(v.data(), v.size(), eDT, true, nodata);
    ...
    {
        std::vector<T> v{cpl::CPLNumericLimits<T>::quiet_NaN(),
                         cpl::CPLNumericLimits<T>::quiet_NaN(),
                         max_v,
                         cpl::CPLNumericLimits<T>::quiet_NaN(),
                         min_v,
                         cpl::CPLNumericLimits<T>::quiet_NaN()};
    ...
    {
        std::vector<T> v{max_v, cpl::CPLNumericLimits<T>::quiet_NaN(), min_v};
        auto idx_min = gdal::min_element(v.data(), v.size(), eDT, false, 0);
        EXPECT_EQ(v[idx_min], min_v);
        auto idx_max = gdal::max_element(v.data(), v.size(), eDT, false, 0);
        EXPECT_EQ(v[idx_max], max_v);
    ...
    {
        std::vector<T> v(257, cpl::CPLNumericLimits<T>::quiet_NaN());
    ...
    {
        std::vector<T> v(255, cpl::CPLNumericLimits<T>::quiet_NaN());
        v[v.size() - 2] = min_v;
        v.back() = max_v;
        auto idx_min = gdal::min_element(v.data(), v.size(), eDT, false, 0);
        EXPECT_EQ(v[idx_min], min_v);
...
TEST_F(test_gdal_minmax_element, float64)
{
    ...
    {
        std::vector<T> v(33, cpl::CPLNumericLimits<T>::quiet_NaN());
        v[5] = min_v;
        v[15] = max_v;
        auto idx_min = gdal::min_element(v.data(), v.size(), eDT, false, 0);
        EXPECT_EQ(v[idx_min], min_v);

...
TEST_F(test_cpl, CPLJSonStreamingParser)
{
    // nominal cases
    {
        ...
        ASSERT_TRUE(oParser.Parse(sText, true));
        ASSERT_EQ(oParser.GetSerialized(), sText);

        oParser.Reset();
        for (size_t i = 0; sText[i]; i++)
            ASSERT_TRUE(oParser.Parse(std::string_view(sText + i, 1),
                                      sText[i + 1] == 0));
        ASSERT_EQ(oParser.GetSerialized(), sText);
    }
    {
    ...
    {
        CPLJSonStreamingParserDump oParser;
        ASSERT_TRUE(!oParser.Parse("tru", true));
        ASSERT_TRUE(!oParser.GetException().empty());
    }
    {
        CPLJSonStreamingParserDump oParser;
        ASSERT_TRUE(!oParser.Parse("tru1", true));
        ASSERT_TRUE(!oParser.GetException().empty());
    }
    {
        CPLJSonStreamingParserDump oParser;
        ASSERT_TRUE(!oParser.Parse("truxe", true));
        ASSERT_TRUE(!oParser.GetException().empty());
    }
    {
        CPLJSonStreamingParserDump oParser;
    ...
    {
        CPLJSonStreamingParserDump oParser;
        ASSERT_TRUE(!oParser.Parse("\"\\u000", true));
        ASSERT_TRUE(!oParser.GetException().empty());
    }
    {
        CPLJSonStreamingParserDump oParser;
        ASSERT_TRUE(!oParser.Parse("\"\\uD834\\ux\"", true));
        ASSERT_TRUE(!oParser.GetException().empty());
    }
    {
        CPLJSonStreamingParserDump oParser;
    ...
    {
        CPLJSonStreamingParserDump oParser;
        ASSERT_TRUE(!oParser.Parse("[true,]", true));
        ASSERT_TRUE(!oParser.GetException().empty());
    }
    {
        CPLJSonStreamingParserDump oParser;
        ASSERT_TRUE(!oParser.Parse("[true,,true]", true));
        ASSERT_TRUE(!oParser.GetException().empty());
    }
    {
        CPLJSonStreamingParserDump oParser;

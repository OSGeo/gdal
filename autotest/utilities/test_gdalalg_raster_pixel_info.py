...
def test_gdalalg_raster_pixel_info_missing_position():
    ...
    with pytest.raises(
        Exception, match="Argument 'position' or 'coordinate-dataset' must be specified"
    ):
        alg.Run()
...
def test_gdalalg_raster_pixel_info_byte_csv():
    ...
    assert (
        ...
        == "geom_x,geom_y,column,line,band_1_raw_value,band_1_unscaled_value,extra_content\n5,10,5,10,132,132,\n"
    )
...
def test_gdalalg_raster_pixel_info_complex_csv():
    ...
    assert (
        alg["output-string"].replace("\r\n", "\n")
        == "geom_x,geom_y,column,line,band_1_real_value,band_1_imaginary_value,extra_content\n5,10,5,10,132,0,\n"
    )
...
def test_gdalalg_raster_pixel_info_from_command_line_csv(gdal_path):
    ...
    assert (
        ret
        == "geom_x,geom_y,column,line,band_1_raw_value,band_1_unscaled_value,extra_content\n5.5,10.5,5.5,10.5,132,132,foo bar"
    )
...
def test_gdalalg_raster_pixel_info_from_to_vector_dataset(tmp_vsimem):

    coordinate_dataset = gdal.GetDriverByName("MEM").CreateVector("")
    srs = osr.SpatialReference(epsg=4267)
    srs.SetAxisMappingStrategy(osr.OAMS_TRADITIONAL_GIS_ORDER)
    coordinate_lyr = coordinate_dataset.CreateLayer("coords", srs=srs)
...
def test_gdalalg_raster_pixel_info_from_vector_dataset_to_geojson(tmp_vsimem):
    ...
    coordinate_dataset = gdal.GetDriverByName("MEM").CreateVector("")
    coordinate_lyr = coordinate_dataset.CreateLayer("coords")
...
def test_gdalalg_raster_pixel_info_coordinate_from_vector_dataset_errors():
    ...
    with pytest.raises(Exception, match="/i/do_not_exist"):
        gdal.alg.raster.pixel_info(
            input="../gcore/data/byte.tif", coordinate_dataset="/i/do_not_exist"
        )
    ...
    with pytest.raises(Exception, match="has more than one vector layer"):
        gdal.alg.raster.pixel_info(
            input="../gcore/data/byte.tif", coordinate_dataset=two_layers_ds
        )

    with pytest.raises(Exception, match="Cannot find layer 'i_do_not_exist'"):
        gdal.alg.raster.pixel_info(
            input="../gcore/data/byte.tif",
            coordinate_dataset=two_layers_ds,

...
# Purpose:  'gdal vector geom make-valid' testing
...
def get_alg():
    return gdal.GetGlobalAlgorithmRegistry()["vector"]["geom"]["make-valid"]
...
def test_gdalalg_vector_geom_make_valid_test_ogrsf(tmp_path):
    ...
    open(gdalg_filename, "wb").write(
        b'{"type": "gdal_streamed_alg","command_line": "gdal vector geom make-valid ../ogr/data/poly.shp --output-format=stream foo","relative_paths_relative_to_this_file":false}'
    )

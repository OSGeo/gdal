...
# Purpose:  'gdal vector geom simplify-coverage' testing
...
@pytest.fixture()
def alg():
    return gdal.GetGlobalAlgorithmRegistry()["vector"]["geom"]["simplify-coverage"]
...
def test_gdalalg_vector_geom_simplify_coverage(alg):
...
def test_gdalalg_vector_geom_simplify_coverage_active_layer(alg):
...
def test_gdalalg_vector_geom_simplify_coverage_no_tolerance(alg):
...
def test_gdalalg_vector_geom_simplify_coverage_bad_tolerance(alg, tol):
...
def test_gdalalg_vector_geom_simplify_coverage_test_ogrsf(tmp_path):
    ...
    open(gdalg_filename, "wb").write(
        b'{"type": "gdal_streamed_alg","command_line": "gdal vector geom simplify-coverage --tolerance 10 ../ogr/data/poly.shp --output-format=stream dummy_dataset_name","relative_paths_relative_to_this_file":false}'
    )

...
def create_index_p_test_file(tmp_path):
    drv = ogr.GetDriverByName("MapInfo File")
    with drv.CreateDataSource(tmp_path / "index_p.mif") as p_ds:
    ...
    ogr.GetDriverByName("MapInfo File").DeleteDataSource(tmp_path / "index_p.mif")


@contextlib.contextmanager
def create_join_t_test_file(tmp_path, create_index=False):
    drv = ogr.GetDriverByName("ESRI Shapefile")
    with drv.CreateDataSource(tmp_path / "join_t.dbf") as s_ds:
    ...
    ogr.GetDriverByName("ESRI Shapefile").DeleteDataSource(tmp_path / "join_t.dbf")
...
def test_ogr_index_can_join_without_index(tmp_path):
    expect = ["Value 5", "Value 10", "Value 9", "Value 4", "Value 3", "Value 1"]

    with create_index_p_test_file(tmp_path), create_join_t_test_file(tmp_path):
...
def test_ogr_index_creating_index_causes_index_files_to_be_created(tmp_path):
    with create_join_t_test_file(tmp_path, create_index=True):
        for filename in [tmp_path / "join_t.idm", tmp_path / "join_t.ind"]:
...
def test_ogr_index_indexed_single_integer_lookup_works(tmp_path):
    expect = ["Value 5"]

    with create_join_t_test_file(tmp_path, create_index=True):
        s_ds = ogr.OpenShared(tmp_path / "join_t.dbf", update=1)
...
def test_ogr_index_indexed_single_string_works(tmp_path):
    expect = [5]
...
def test_ogr_index_unimplemented_range_query_works(tmp_path):
    expect = [0, 1, 2]
...
def test_ogr_index_indexed_join_works(tmp_path):
    expect = ["Value 5", "Value 10", "Value 9", "Value 4", "Value 3", "Value 1"]

    with create_index_p_test_file(tmp_path), create_join_t_test_file(
        tmp_path, create_index=True
    ):
        p_ds = ogr.OpenShared(tmp_path / "index_p.mif", update=0)
        with p_ds.ExecuteSQL(
            "SELECT * FROM index_p p "
...
def test_ogr_index_drop_index_removes_files(tmp_path):
    with create_join_t_test_file(tmp_path, create_index=True):
        with ogr.OpenShared(tmp_path / "join_t.dbf", update=1) as s_ds:
...
def test_ogr_index_attribute_filter_works_after_drop_index(tmp_path):
    expect = ["Value 5"]
...
def test_ogr_index_recreating_index_causes_index_files_to_be_created(tmp_path):
...
def test_ogr_index_recreating_index_causes_index_to_be_populated(tmp_path):
    ...
    with create_join_t_test_file(tmp_path, create_index=True):
        ...
        with open(tmp_path / "join_t.idm", "rt") as f:
            xml = f.read()
...
def test_ogr_index_creating_index_in_separate_steps_works(tmp_path):

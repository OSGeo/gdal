...
def ogr_interlis_arc1():
    ...
    if abs(length-length_0_1_deg) > 0.7: #72.7016803283
        gdaltest.post_reason( 'Length() result wrong, got %g.' % length )
        return 'fail'
    if ogrtest.check_feature_geometry(geom, 'LINESTRING (186.38 206.82,186.73341431008015 203.848245572904574,186.774259656118232 200.85582887277107,186.50208852795555 197.875535443169667,185.919882889427782 194.940018002581496,185.03402150734496 192.081438694382229,183.854210064387615 189.331116711668642,183.26 188.19,181.68314072585477 185.646431467588712,179.849044341893887 183.281623546451669,177.777805591909413 181.121485567040764,175.492117401080691 179.189684453096334,173.017022247537511 177.507385421878126,170.37963779172901 176.093020093615877,170.18 176.0,140.69 156.63)') != 0:
    ...
    if abs(length-length_0_1_deg) > 0.7: #72.6745269621
        gdaltest.post_reason( 'Length() result wrong, got %g.' % length )
        return 'fail'
    if ogrtest.check_feature_geometry(geom, 'LINESTRING (186.38 206.82,186.795472728605944 201.853573833202745,186.342222639520514 196.890452757185358,185.03402150734496 192.081438694382229,183.26 188.19,180.48780698927294 184.04838770890612,177.038546392511279 180.451082079119971,173.017022247537511 177.507385421878126,170.18 176.0,140.69 156.63)') != 0:
        gdaltest.post_reason( '<gml:Curve> not correctly parsed' )
        print(geom.ExportToWkt())
        return 'fail'
    points = geom.GetPoints()

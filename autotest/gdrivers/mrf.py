...
def test_mrf_zen_test(tmp_path):
...
<VRTDataset rasterXSize="512" rasterYSize="512">
  <VRTRasterBand dataType="Byte" band="1">
    ...
    <ComplexSource>
      <SourceFilename relativeToVRT="0">{tmp_path}/masked.mrf</SourceFilename>
...
"""
    for interleave in "PIXEL", "BAND":
        ...
        for f in glob.glob(str(tmp_path / "masked.*")):
            gdal.Unlink(f)
...
def test_mrf_cached_source(tmp_path):
    ...
    with gdal.config_option("MRF_ENABLE_CACHING", "ON"):
        ...
        open(tmp_path / "byte.tif", "wb").write(open("data/byte.tif", "rb").read())
        gdal.Translate(
            tmp_path / "out.mrf",
            tmp_path / "byte.tif",
            format="MRF",
        ...
        ds = gdal.Open(str(tmp_path / "out.mrf"))
        cs = ds.GetRasterBand(1).Checksum()
        expected_cs = 4672
        assert cs == expected_cs
        ds = None

        gdal.Unlink(tmp_path / "byte.tif")
        ds = gdal.Open(tmp_path / "out.mrf")
        cs = ds.GetRasterBand(1).Checksum()
        expected_cs = 4672
        assert cs == expected_cs
        ds = None
        cleanup(str(tmp_path / "out."))

        # Caching MRF in mp_safe mode
        open(tmp_path / "byte.tif", "wb").write(open("data/byte.tif", "rb").read())
        open(tmp_path / "out.mrf", "wt").write("""<MRF_META>
    <CachedSource>
    ...
    <Raster mp_safe="on">
        ...
        # Cloning MRF
        open(tmp_path / "cloning.mrf", "wt").write("""<MRF_META>
    <CachedSource>
    ...
    <Raster>
        ...
        ds = gdal.Open(tmp_path / "cloning.mrf")
        cs = ds.GetRasterBand(1).Checksum()
        expected_cs = 4672
        assert cs == expected_cs
        ds = None
        cleanup(tmp_path / "out.")
...
def test_mrf_cleanup(tmp_path):
    ...
    for f in (fname for n in files for fname in glob.glob(str(tmp_path) + "/" + n)):
        gdal.Unlink(f)

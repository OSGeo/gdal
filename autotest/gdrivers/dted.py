...
pytestmark = pytest.mark.require_driver("DTED")
...
def test_dted_5(tmp_path):
    ...
    ds = driver.Create(str(tmp_path / "n43.dt1.tif"), 1201, 1201, 1, gdal.GDT_Int16)
    ds.SetProjection(
        'GEOGCS["WGS 84",DATUM["WGS_1984",SPHEROID["WGS 84",6378137,298.257223563,AUTHORITY["EPSG","7030"]],AUTHORITY["EPSG","6326"]],PRIMEM["Greenwich",0,AUTHORITY["EPSG","8901"]],UNIT["degree",0.0174532925199433,AUTHORITY["EPSG","9122"]],AXIS["Latitude",NORTH],AXIS["Longitude",EAST],AUTHORITY["EPSG","4326"]]'
    )
    ref_geotransform = (
    ...
    ds = gdal.Open(str(tmp_path / "n43.dt1.tif"))
...
def test_dted_6(tmp_path):
    ...
    driver = gdal.GetDriverByName("GTiff")
    ds = driver.Create(str(tmp_path / "n43.dt2.tif"), 3601, 3601, 1, gdal.GDT_Int16)
    ds.SetProjection(
        'GEOGCS["WGS 84",DATUM["WGS_1984",SPHEROID["WGS 84",6378137,298.257223563,AUTHORITY["EPSG","7030"]],AUTHORITY["EPSG","6326"]],PRIMEM["Greenwich",0,AUTHORITY["EPSG","8901"]],UNIT["degree",0.0174532925199433,AUTHORITY["EPSG","9122"]],AXIS["Latitude",NORTH],AXIS["Longitude",EAST],AUTHORITY["EPSG","4326"]]'
    )
    ref_geotransform = (
    ...
    ds = gdal.Open(str(tmp_path / "n43.dt2.tif"))
    geotransform = ds.GetGeoTransform()
    for i in range(6):
        assert geotransform[i] == pytest.approx(ref_geotransform[i], abs=1e-10)
...
def test_dted_9(tmp_path):
    ...
    driver = gdal.GetDriverByName("GTiff")
    dsDst = driver.Create(str(tmp_path / "n53.dt1.tif"), 601, 1201, 1, gdal.GDT_Int16)
    dsDst.SetProjection(
        'GEOGCS["WGS 84",DATUM["WGS_1984",SPHEROID["WGS 84",6378137,298.257223563,AUTHORITY["EPSG","7030"]],AUTHORITY["EPSG","6326"]],PRIMEM["Greenwich",0,AUTHORITY["EPSG","8901"]],UNIT["degree",0.0174532925199433,AUTHORITY["EPSG","9122"]],AXIS["Latitude",NORTH],AXIS["Longitude",EAST],AUTHORITY["EPSG","4326"]]'
    )
    ...
    ds = gdal.Open(str(tmp_path / "n53.dt1.tif"))
    driver = gdal.GetDriverByName("DTED")
    dsDst = driver.CreateCopy(str(tmp_path / "n53.dt1"), ds)
...
def test_dted_cleanup(tmp_path):
    try:
        os.remove(str(tmp_path / "n43.dt1.tif"))
        os.remove(str(tmp_path / "n43.dt1.aux.xml"))
        os.remove(str(tmp_path / "n43.dt1"))
        os.remove(str(tmp_path / "n53.dt1.tif"))
        os.remove(str(tmp_path / "n53.dt1.aux.xml"))
        os.remove(str(tmp_path / "n53.dt1"))
        os.remove(str(tmp_path / "n43.dt2.tif"))
        os.remove(str(tmp_path / "n43.dt2.aux.xml"))
        os.remove(str(tmp_path / "n43.dt2"))
    except OSError:
        pass

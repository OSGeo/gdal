...
def _test_jp2metadata(file_path, rpc_values_to_check=None):
    ...
    assert (
        "IMD" in mddlist and "RPC" in mddlist and "IMAGERY" in mddlist
...
def test_jp2metadata_2b():
    ...
    assert (
        md[
            "Geometric_Data.Use_Area.Located_Geometric_Values_1.Ground_Sample_Distance.GSD_ACROSS_TRACK"
        ]
        == "0.7054510257833884"
    )
    assert (
        md[
            "Geometric_Data.Use_Area.Located_Geometric_Values_2.Ground_Sample_Distance.GSD_ACROSS_TRACK"
        ]
        == "0.7059011471512184"
    )
...
def test_jp2metadata_3():
    ...
    gt = ds.GetGeoTransform()
    if (
        gt[0] != pytest.approx(8, abs=0.0000001)
        or gt[3] != pytest.approx(50, abs=0.000001)
...
def test_jp2metadata_getjpeg2000structure():
    ...
    ret = gdal.GetJPEG2000StructureAsString(
        "data/jpeg2000/byte_tlm_plt.jp2", ["ALL=YES"]
    )
    assert ret is not None

    ret = gdal.GetJPEG2000StructureAsString(
        "data/jpeg2000/byte_one_poc.j2k", ["ALL=YES"]
    )
    assert ret is not None

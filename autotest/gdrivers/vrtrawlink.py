...
def test_vrtrawlink_non_relative_filename():
    ...
    with pytest.raises(Exception, match="relativeToVRT flag not being set"):
        gdal.Open(
            """<VRTDataset rasterXSize="1" rasterYSize="1">
            <VRTRasterBand dataType="Byte" band="1" subClass="VRTRawRasterBand">
...
def test_vrtrawlink_relative_filename_with_abspath():
    ...
    with gdaltest.config_option(
        ...
        with pytest.raises(
            Exception, match="is rejected because it is not relative to the VRT path "
        ):
            gdal.Open(
                """<VRTDataset rasterXSize="1" rasterYSize="1">
                <VRTRasterBand dataType="Byte" band="1" subClass="VRTRawRasterBand">
...
def test_vrtrawlink_GDAL_VRT_RAWRASTERBAND_ALLOWED_SOURCE_path_escape():
    ...
    with gdaltest.config_option("GDAL_VRT_RAWRASTERBAND_ALLOWED_SOURCE", os.getcwd()):
        with pytest.raises(
            Exception,
            match="is rejected because the presence of ../ in it may escape from the allowed path",
        ):
            gdal.Open(
                """<VRTDataset rasterXSize="4" rasterYSize="3">
        <VRTRasterBand dataType="Float32" band="1" subClass="VRTRawRasterBand">
...
def test_vrtrawlink_GDAL_VRT_RAWRASTERBAND_ALLOWED_SOURCE_path_not_contained():
    ...
    with gdaltest.config_option("GDAL_VRT_RAWRASTERBAND_ALLOWED_SOURCE", os.getcwd()):
        with pytest.raises(
            Exception,
            match="'/etc/passwd' is rejected because it is not contained in one of the allowed path",
        ):
            gdal.Open(
                """<VRTDataset rasterXSize="1" rasterYSize="1">
            <VRTRasterBand dataType="Byte" band="1" subClass="VRTRawRasterBand">

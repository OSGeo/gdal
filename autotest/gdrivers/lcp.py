...
def test_lcp_7(tmp_path):
    ...
    for i in [5, 7, 8, 10]:
        ...
        dst_ds = lcp_drv.CreateCopy(tmp_path / "lcp_7.lcp", src_ds, False, co)
...
def test_lcp_8(tmp_path):
    ...
    with gdal.quiet_errors():
        ...
        for i in [0, 1, 2, 3, 4, 6, 9, 11]:
            ...
            assert src_ds is not None
            dst_ds = lcp_drv.CreateCopy(tmp_path / "lcp_8.lcp", src_ds, False, co)
            src_ds = None
...
def test_lcp_9(tmp_path):
    ...
    co = ["LATITUDE=0", "LINEAR_UNIT=METER"]
    lcp_ds = lcp_drv.CreateCopy(tmp_path / "lcp_9.lcp", src_ds, False, co)
...
def test_lcp_10(tmp_path):
    ...
    for option in ["METERS", "FEET"]:
        ...
        lcp_ds = drv.CreateCopy(tmp_path / "lcp_10.lcp", src_ds, False, co)
        assert lcp_ds is not None
...
def test_lcp_11(tmp_path):
    ...
    for option in ["DEGREES", "PERCENT"]:
        ...
        lcp_ds = drv.CreateCopy(tmp_path / "lcp_11.lcp", src_ds, False, co)
        assert lcp_ds is not None
...
def test_lcp_12(tmp_path):
    ...
    for option in ["GRASS_CATEGORIES", "AZIMUTH_DEGREES", "GRASS_DEGREES"]:
        ...
        lcp_ds = drv.CreateCopy(tmp_path / "lcp_12.lcp", src_ds, False, co)
        assert lcp_ds is not None
...
def test_lcp_13(tmp_path):
    ...
    for option in ["PERCENT", "CATEGORIES"]:
        ...
        lcp_ds = drv.CreateCopy(tmp_path / "lcp_13.lcp", src_ds, False, co)
        assert lcp_ds is not None
...
def test_lcp_14(tmp_path):
    ...
    for option in ["METERS", "FEET", "METERS_X_10", "FEET_X_10"]:
        ...
        lcp_ds = drv.CreateCopy(tmp_path / "lcp_14.lcp", src_ds, False, co)
        assert lcp_ds is not None
...
def test_lcp_15(tmp_path):
    ...
    for option in ["METERS", "FEET", "METERS_X_10", "FEET_X_10"]:
        ...
        lcp_ds = drv.CreateCopy(tmp_path / "lcp_15.lcp", src_ds, False, co)
        assert lcp_ds is not None
...
def test_lcp_16(tmp_path):
    ...
    for i, option in enumerate(
        ...
        lcp_ds = drv.CreateCopy(tmp_path / "lcp_16.lcp", src_ds, False, co)
        assert lcp_ds is not None
...
def test_lcp_17(tmp_path):
    ...
    for i, option in enumerate(["MG_PER_HECTARE_X_10", "TONS_PER_ACRE_X_10"]):
        ...
        lcp_ds = drv.CreateCopy(tmp_path / "lcp_17.lcp", src_ds, False, co)
        assert lcp_ds is not None
...
def test_lcp_18(tmp_path):
    ...
    lcp_ds = drv.CreateCopy(tmp_path / "lcp_18.lcp", src_ds, False, co)
    assert lcp_ds is not None
...
def test_lcp_19(tmp_path):
    ...
    lcp_ds = drv.CreateCopy(tmp_path / "lcp_19.lcp", src_ds, False, co)
    assert lcp_ds is not None
...
def test_lcp_20(tmp_path):
    ...
    lcp_ds = drv.CreateCopy(tmp_path / "lcp_20.lcp", src_ds, False, co)
    assert lcp_ds is not None
...
def test_lcp_21(tmp_path):
    ...
    co = ["LATITUDE=0", "LINEAR_UNIT=METER"]
    lcp_ds = drv.CreateCopy(tmp_path / "lcp_21.lcp", src_ds, False, co)
    assert lcp_ds is not None
    assert [
...
def test_lcp_22(tmp_path):
    ...
    co = ["LATITUDE=0", "LINEAR_UNIT=METER"]
    lcp_ds = drv.CreateCopy(tmp_path / "lcp_22.lcp", src_ds, False, co)
    assert lcp_ds is not None
    for i in range(10):
...
def test_lcp_23(tmp_path):
    ...
    with gdal.quiet_errors():
        for option in [
            ...
            lcp_ds = drv.CreateCopy(tmp_path / "lcp_23.lcp", src_ds, False, co)

...
def test_band_arithmetic_as_type():

    ds = gdal.GetDriverByName("MEM").Create("", 1, 1)
    assert ds.GetRasterBand(1).as_type(gdal.GDT_Float64).DataType == gdal.GDT_Float64

    with pytest.raises(
        Exception, match=r"GDALRasterBandAsDataType\(GDT_Unknown\) not supported"
    ):
        ds.GetRasterBand(1).as_type(gdal.GDT_Unknown)

    with pytest.raises(Exception, match=r"Invalid dt value"):
        assert ds.GetRasterBand(1).as_type("wrong")


def test_band_arithmetic_as_type_numpy():

    np = pytest.importorskip("numpy")
    gdaltest.importorskip_gdal_array()

    ds = gdal.GetDriverByName("MEM").Create("", 1, 1)
    assert ds.GetRasterBand(1).as_type(np.float64).DataType == gdal.GDT_Float64
...
def test_band_arithmetic_rgb_to_greylevel_subfunc():
    def greylevel():
        ...
        return (0.299 * R + 0.587 * G + 0.114 * B).as_type(gdal.GDT_Byte)
...
def test_band_arithmetic_rgb_to_greylevel_with():
    ...
    with gdal.Open("data/rgbsmall.tif") as ds:
        ...
        G = ds.GetRasterBand(2)
        B = ds.GetRasterBand(3)
        greylevel = (0.299 * R + 0.587 * G + 0.114 * B).as_type(gdal.GDT_Byte)
        assert greylevel.Checksum() == 21466

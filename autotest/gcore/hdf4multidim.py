...
def test_hdf4multidim_gdal_sds_2d():
    ...
    ds = gdal.Open("data/byte_2.hdf", gdal.OF_MULTIDIM_RASTER)
    assert ds
    rg = ds.GetRootGroup()
...
def test_hdf4multidim_gdal_sds_3d():
    ...
    ds = gdal.Open("data/byte_3.hdf", gdal.OF_MULTIDIM_RASTER)
    assert ds
    rg = ds.GetRootGroup()
    assert len(rg.GetGroupNames()) == 0
    assert rg.OpenGroup("scientific_datasets") is None
...
def test_hdf4multidim_sds():
    ...
    ds = gdal.Open("data/SDS.hdf", gdal.OF_MULTIDIM_RASTER)
    assert ds
    rg = ds.GetRootGroup()
...
def test_hdf4multidim_sds_unlimited_dim():
    ...
    ds = gdal.Open("data/SDSUNLIMITED.hdf", gdal.OF_MULTIDIM_RASTER)
    assert ds
    rg = ds.GetRootGroup()
    assert rg.GetGroupNames() == ["scientific_datasets"]
    subg = rg.OpenGroup("scientific_datasets")
...
def test_hdf4multidim_gr():
    ...
    ds = gdal.Open("data/General_RImages.hdf", gdal.OF_MULTIDIM_RASTER)
    assert ds
    rg = ds.GetRootGroup()
...
def test_hdf4multidim_gr_palette():
    ...
    def get_lut():
        ds = gdal.Open("data/Image_with_Palette.hdf", gdal.OF_MULTIDIM_RASTER)
        assert ds
        rg = ds.GetRootGroup()
        assert rg.GetGroupNames() == ["general_rasters"]
        subg = rg.OpenGroup("general_rasters")

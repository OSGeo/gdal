...
# Configure GDAL
sh -c "cd $PWD && mkdir build && cd build && cmake .. -DCMAKE_INSTALL_PREFIX=/usr -DGDAL_USE_WEBP=OFF -DGDAL_USE_TIFF_INTERNAL=ON -DGDAL_USE_GEOTIFF_INTERNAL=ON -DUSE_CCACHE=ON -DCMAKE_C_FLAGS='-Werror' -DCMAKE_CXX_FLAGS='-Werror'"

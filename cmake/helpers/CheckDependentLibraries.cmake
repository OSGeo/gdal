...
# First check with CMake config files (starting at version 8, due to issues with static linking with earlier ones), and then fallback to the FindPROJ module.
get_cmake_property(_variableNames VARIABLES)
# Collect all variables related to PROJ
set(_list_proj_variables_specified)
foreach (_variableName ${_variableNames})
    string(REGEX MATCH "^PROJ_" is_proj_prefix "${_variableName}")
    if(is_proj_prefix OR "${_variableName}" STREQUAL "FIND_PACKAGE_MESSAGE_DETAILS_PROJ")
        list(APPEND _list_proj_variables_specified "${_variableName}")
    endif()
endforeach()
find_package(PROJ CONFIG)
if(PROJ_FOUND)
    if(PROJ_VERSION VERSION_GREATER_EQUAL 8.0)
        string(APPEND GDAL_IMPORT_DEPENDENCIES "find_dependency(PROJ ${PROJ_VERSION_MAJOR}.${PROJ_VERSION_MINOR} CONFIG)\n")
    else()
        message(STATUS "PROJ ${PROJ_VERSION} < 8.0 found using CONFIG files. Retrying with MODULE method")
        # Remove all variables related to PROJ that have been set by find_package(PROJ CONFIG)
        unset(HAVE_PROJ)
        get_cmake_property(_variableNames VARIABLES)
        foreach (_variableName ${_variableNames})
            string(REGEX MATCH "^PROJ_" is_proj_prefix "${_variableName}")
            if(is_proj_prefix OR "${_variableName}" STREQUAL "FIND_PACKAGE_MESSAGE_DETAILS_PROJ")
               list(FIND _list_proj_variables_specified ${_variableName} _index)
               if(_index EQUAL -1)
...
if(NOT PROJ_FOUND)
  ...
  string(APPEND GDAL_IMPORT_DEPENDENCIES "find_dependency(PROJ ${PROJ_VERSION_MAJOR}.${PROJ_VERSION_MINOR})\n")
endif()

...
#backport# cmake_policy (GET CMP0094 _${_PYTHON_PREFIX}_LOOKUP_POLICY)
set(${_PYTHON_PREFIX}_LOOKUP_POLICY OLD)

cmake_policy (VERSION 3.5)

#backport# if (_${_PYTHON_PREFIX}_LOOKUP_POLICY)
#backport#   cmake_policy (SET CMP0094 ${_${_PYTHON_PREFIX}_LOOKUP_POLICY})
#backport# endif()
include(${CMAKE_CURRENT_LIST_DIR}/ListExt.cmake)
...
function (_PYTHON_GET_ABIFLAGS _PGABIFLAGS)
  ...
  if (pydebug STREQUAL "ON")
    if (abiflags)
      #backport# list (TRANSFORM abiflags PREPEND "d")
      list_transform_prepend(abiflags "d")
    else()
      set (abiflags "d")
    endif()
  elseif (pydebug STREQUAL "ANY")
    if (abiflags)
      set (flags "${abiflags}")
      #backport# list (TRANSFORM flags PREPEND "d")
      list_transform_prepend(flags "d")
...
function (_PYTHON_GET_PATH_SUFFIXES _PYTHON_PGPS_PATH_SUFFIXES)
  ...
  if (_PGPS_LIBRARY)
    ...
    if (CMAKE_LIBRARY_ARCHITECTURE)
      set (suffixes "${abi}")
      if (suffixes)
        #backport# list (TRANSFORM suffixes PREPEND "lib/python${_PGPS_VERSION}/config-${_PGPS_VERSION}")
        #backport# list (TRANSFORM suffixes APPEND "-${CMAKE_LIBRARY_ARCHITECTURE}")
        list_transform_prepend(suffixes "lib/python${_PGPS_VERSION}/config-${_PGPS_VERSION}")
        list_transform_append(suffixes "-${CMAKE_LIBRARY_ARCHITECTURE}")
      else()
  ...
  elseif (_PGPS_INCLUDE)
    set (suffixes "${abi}")
    if (suffixes)
      #backport# list (TRANSFORM suffixes PREPEND "include/python${_PGPS_VERSION}")
      list_transform_prepend(suffixes "include/python${_PGPS_VERSION}")
    else()
  ...
  if (_PGN_POSIX)
    ...
    if (abi)
      if (_PGN_CONFIG AND DEFINED CMAKE_LIBRARY_ARCHITECTURE)
        set (abinames "${abi}")
        #backport# list (TRANSFORM abinames PREPEND "${CMAKE_LIBRARY_ARCHITECTURE}-python${_PGN_VERSION}")
        #backport# list (TRANSFORM abinames APPEND "-config")
        list_transform_prepend(abinames "${CMAKE_LIBRARY_ARCHITECTURE}-python${_PGN_VERSION}")
        list_transform_append(abinames "-config")
        list (APPEND names ${abinames})
      endif()
      set (abinames "${abi}")
      #backport# list (TRANSFORM abinames PREPEND "python${_PGN_VERSION}")
      list_transform_prepend(abinames "python${_PGN_VERSION}")
      if (_PGN_CONFIG)
        #backport# list (TRANSFORM abinames APPEND "-config")
        list_transform_append(abinames "-config")
      endif()
...
if (_${_PYTHON_PREFIX}_REQUIRED_VERSION_MAJOR VERSION_LESS 3)
  ...
  if (DEFINED ${_PYTHON_PREFIX}_FIND_ABI)
    ...
    #backport# list (TRANSFORM _${_PYTHON_PREFIX}_FIND_ABI REPLACE "^(TRUE|Y(ES)?|1)$" "ON")
    #backport# list (TRANSFORM _${_PYTHON_PREFIX}_FIND_ABI REPLACE "^(FALSE|N(O)?|0)$" "OFF")
    list_transform_replace(_${_PYTHON_PREFIX}_FIND_ABI "^(TRUE|Y(ES)?|1)$" "ON")
    list_transform_replace(_${_PYTHON_PREFIX}_FIND_ABI "^(FALSE|N(O)?|0)$" "OFF")

        ...
        else
        {
                 ...
                 ++iField)
            {
                ...
                if (!osFilter.empty())
                    osFilter += " AND ";
                osFilter += '"';
                osFilter += CPLString(m_key[iField]).replaceAll('"', "\"\"");
                osFilter += "\" = ";
                if (keyFieldTypes[iField] == OFTString)
                {
                    osFilter += '\'';
                    osFilter +=
                        CPLString(poSrcFeature->GetFieldAsString(nSrcFieldIdx))
                            .replaceAll('\'', "''");
                    osFilter += '\'';
                }
                else if (keyFieldTypes[iField] == OFTReal)
                {
                    osFilter += CPLSPrintf(
                        "%.17g", poSrcFeature->GetFieldAsDouble(nSrcFieldIdx));
                }
                else
                {
                    osFilter += CPLSPrintf(
            ...
            if (poDstFeature)
            {
                ...
                {
                    CPLDebugOnly("GDAL",
                                 "Updating dest feature " CPL_FRMT_GIB
                                 " with src feature " CPL_FRMT_GIB,
                                 poDstFeature->GetFID(),
                                 poSrcFeature->GetFID());
                }
            }

    ...
    if (bIsGenericPipeline)
    {
             ...
             !forAutoComplete && i < steps.size(); ++i)
        {
                ...
                GetStepAlg(steps[i].alg->GetName()) == nullptr)
            {
                ...
                if (steps[i].alg->GetName() == GDALTeeStepAlgorithm::NAME)
                {
                    const auto poSrcTeeAlg =
                        dynamic_cast<const GDALTeeStepAlgorithm *>(
                            steps[i].alg.get());
                    auto poDstTeeAlg =
                        dynamic_cast<GDALTeeStepAlgorithm *>(newAlg.get());
    ...
    std::vector<std::string> &nestedPipelineArgs, bool forAutoComplete)
{
    ...
    if (curAlg->GetName() == GDALTeeStepAlgorithm::NAME)
    ...
    if (curAlg->GetName() == GDALTeeStepAlgorithm::NAME)
    {
        auto teeAlg = dynamic_cast<GDALTeeStepAlgorithm *>(curAlg);
        if (teeAlg)
        {

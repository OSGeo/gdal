          ...
          ConstructorOptions().SetAddDefaultArguments(false))
{
    ...
    AddArg("gcp", 0,
        ...
        .AddValidationAction(
            [this]()
            {
                if (!(m_gcps.size() == 1 && !m_gcps[0].empty() &&
                      m_gcps[0][0] == '@'))
                {
...
std::vector<gdal::GCP> GDALRasterEditAlgorithm::ParseGCPs() const
{
    ...
    if (m_gcps.size() == 1 && !m_gcps[0].empty() && m_gcps[0][0] == '@')
    {
        auto poDS = std::unique_ptr<GDALDataset>(
            GDALDataset::Open(m_gcps[0].substr(1).c_str(),
        ...
        if (poDS->GetLayerCount() != 1)
        {
            ReportError(CE_Failure, CPLE_AppDefined,
                        "Only single-layer dataset support to specify GCPs");
            return ret;
        }

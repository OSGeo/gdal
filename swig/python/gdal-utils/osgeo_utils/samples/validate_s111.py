...
#
# Project:  GDAL/OGR
# Purpose:  Test compliance of IHO S111 v2.0 dataset
...
# Validates against
# https://registry.iho.int/productspec/view.do?idx=209&product_ID=S-111&statusS=5&domainS=ALL&category=product_ID&searchValue= and
# https://iho.int/uploads/user/pubs/standards/s-100/S-100_5.2.0_Final_Clean.pdf

# "111_DevXXXX" are for traceability with respect to requirements of the spreadsheet:
# https://github.com/iho-ohi/S-100-Validation-Checks/blob/main/Documents/S-158-111/0.2.0/S-158_111_0_2_0_20241209.xlsx
...
class S111ValidationException(Exception):
    pass
...
class S111Checker:
    ...
    def _error(self, msg):
        ...
        if self.abort_at_first_error:
            raise S111ValidationException(f"{ERROR}: {msg}")

    def _critical_error(self, msg):
        self.errors += [(CRITICAL_ERROR, msg)]
        if self.abort_at_first_error:
            raise S111ValidationException(f"{CRITICAL_ERROR}: {msg}")
    ...
    def _check_attributes(self, ctxt_name, group, attr_list):
        ...
        for attr_def in attr_list:
            if attr_def.required and attr_def.name not in group.attrs:
                # 111_Dev1002: check presence of required attributes
                self._error(
    ...
    def check(self):
        ...
        topLevelAttributesList = [
            ...
            AttributeDefinition(
                name="surfaceCurrentDepth",
                required=True,
                type="float32",
                fixed_value=None,
            ),
            AttributeDefinition(
                name="verticalCS", required=False, type="int32", fixed_value=None
            ),
            AttributeDefinition(
            ...
            # S111_Dev1003
            AttributeDefinition(
                name="verticalDatum", required=False, type="int32", fixed_value=None
            ),
        ]

        self._log_check("111_Dev1002")
        self._log_check("111_Dev1003")
        self._check_attributes("top level", f, topLevelAttributesList)

        self._log_check("111_Dev1004")
        if "epoch" in f.attrs and not f.attrs["epoch"]:
            self._warning("Attribute epoch present but empty or blank.")

        self._log_check("111_Dev1005")
        ...
        depthTypeIndex = _get_int_attr_or_none(f, "depthTypeIndex")
        if depthTypeIndex == 1:
            if "verticalCS" not in f.attrs:
                self._error(
                    "verticalCS missing, but mandatory because depthTypeIndex = 1"
                )
            if "verticalDatumReference" not in f.attrs:
                self._error(
                    "verticalDatumReference missing, but mandatory because depthTypeIndex = 1"
                )
            if "verticalDatum" not in f.attrs:
                self._error(
                    "verticalDatum missing, but mandatory because depthTypeIndex = 1"
                )

        self._log_check("111_Dev1007")
        ...
        if "SurfaceCurrent" in f.keys():
            self._validate_SurfaceCurrent(f)
        else:
            self._critical_error("Missing /SurfaceCurrent group")
    ...
    def _validate_group_f_featureCode(self, rootGroup, group_f, featureCode):
        ...
        self._log_check("111_Dev1009")
        values = set([v.decode("utf-8") for v in featureCode[:]])
        if "SurfaceCurrent" not in values:
            self._critical_error(
                "SurfaceCurrent feature missing from featureCode array"
            )

        self._log_check("111_Dev1010")
        for value in values:
            if value not in ("SurfaceCurrent",):
                #
                self._critical_error(
        ...
        if "SurfaceCurrent" in group_f.keys():
            self._validate_group_f_SurfaceCurrent(group_f)

    def _validate_group_f_SurfaceCurrent(self, group_f):
        self._log_check("111_Dev1012")

        SurfaceCurrent = group_f["SurfaceCurrent"]
        if not isinstance(SurfaceCurrent, h5py.Dataset):
            self._critical_error("'/Group_F/SurfaceCurrent' is not a dataset")
        elif len(SurfaceCurrent.shape) != 1:
            self._critical_error(
                "'/Group_F/SurfaceCurrent' is not a one-dimensional dataset"
            )
        elif SurfaceCurrent.dtype != [
            ("code", "O"),
            ("name", "O"),
            ("uom.name", "O"),
            ("fillValue", "O"),
            ...
            self._critical_error("'/Group_F/SurfaceCurrent' has not expected data type")
        else:
            self._log_check("111_Dev1013")

            if SurfaceCurrent.shape not in ((2,), (3,), (4,), (5,), (6,)):
                self._critical_error(
                    "'/Group_F/SurfaceCurrent' is not of shape 2, 3, 4, 5 or 6"
                )

            type = SurfaceCurrent.id.get_type()
            ...
            for member_idx in range(type.get_nmembers()):
                ...
                if not isinstance(subtype, h5py.h5t.TypeStringID):
                    self._critical_error(
                        f"Member of index {member_idx} in /Group_F/SurfaceCurrent is not a string"
                    )
                    return
                if not subtype.is_variable_str():
                    self._critical_error(
                        f"Member of index {member_idx} in /Group_F/SurfaceCurrent is not a variable length string"
                    )

            values = SurfaceCurrent[:]
            # Table 10-3 - Sample contents of the one-dimensional compound array
            expected_values = [
                [
                    "surfaceCurrentSpeed",
            ...
            for row in range(SurfaceCurrent.shape[0]):
                row_content = [values[row][col].decode("utf-8") for col in range(8)]
                if row_content not in expected_values:
                    self._critical_error(
                        f"/Group_F/SurfaceCurrent: row {row}, got value '{row_content}', which is not in '{expected_values}'"
                    )

    def _validate_SurfaceCurrent(self, f):
        SurfaceCurrent = f["SurfaceCurrent"]
        if not isinstance(SurfaceCurrent, h5py.Group):
            self._critical_error("/SurfaceCurrent is not a group")
            return

        # Cf Table 12-2 - Feature Type metadata, pertaining to the SurfaceCurrent feature type

        self._log_check("111_Dev2002")  # for dimension
        attr_list = [
            AttributeDefinition(
        ...
        dataCodingFormat = _get_int_attr_or_none(SurfaceCurrent, "dataCodingFormat")
        if dataCodingFormat not in (1, 2, 3, 4, 8):
            self._error(f"dataCodingFormat={dataCodingFormat} is invalid")

        if dataCodingFormat == 2:  # regular grid
            attr_list += [
                AttributeDefinition(
        ...
        elif dataCodingFormat == 3:  # ungeorectified grid
            attr_list += [
                AttributeDefinition(
                    name="interpolationType",
                    required=True,
        ...
        self._log_check("111_Dev2001")
        self._log_check("111_Dev2008")
        self._log_check("111_Dev2009")
        self._log_check("111_Dev2018")
        self._log_check("111_Dev2019")
        self._check_attributes("SurfaceCurrent group", SurfaceCurrent, attr_list)

        self._log_check("111_Dev2003")
        if "commonPointRule" in SurfaceCurrent.attrs:
            expected_values = {
                1: "average",
                2: "low",
                3: "high",
            ...
            self._validate_enumeration(
                SurfaceCurrent, "commonPointRule", expected_values
            )

            self._log_check("111_Dev2004")
            commonPointRule = _get_int_attr_or_none(SurfaceCurrent, "commonPointRule")
            if commonPointRule != 3:
                self._warning(
                    '/SurfaceCurrent["commonPointRule"] attribute value is not the recommended value 3 (high)'
                )

        if "dataCodingFormat" in SurfaceCurrent.attrs:
            expected_values = {
                1: "Fixed Stations",
                2: "Regular Grid",
        ...
        if horizontalPositionUncertainty and not (
            ...
            self._warning(
                '/SurfaceCurrent["horizontalPositionUncertainty"] attribute value must be -1 or positive'
            )

        verticalUncertainty = _get_float_attr_or_none(
            SurfaceCurrent, "verticalUncertainty"
        )
        if verticalUncertainty and not (
            verticalUncertainty == -1.0 or verticalUncertainty >= 0
        ):
            self._warning(
                '/SurfaceCurrent["verticalUncertainty"] attribute value must be -1 or positive'
            )

        self._log_check("111_Dev2006")
        timeUncertainty = _get_float_attr_or_none(SurfaceCurrent, "timeUncertainty")
        if timeUncertainty and not (timeUncertainty == -1.0 or timeUncertainty >= 0):
            self._warning(
                '/SurfaceCurrent["timeUncertainty"] attribute value must be -1 or positive'
            )

        self._log_check("111_Dev2007")
        numInstances = _get_int_attr_or_none(SurfaceCurrent, "numInstances")
        if numInstances is not None:
            if numInstances <= 0:
                self._critical_error(
                    '/SurfaceCurrent["numInstances"] attribute value must be >= 1'
                )
                numInstances = None

        scanDirection_values = None
        if "sequencingRule.scanDirection" in SurfaceCurrent.attrs:
            scanDirection = SurfaceCurrent.attrs["sequencingRule.scanDirection"]
            if isinstance(scanDirection, str):
                ...
                self._log_check("111_Dev2016")
                if len(scanDirection_values) != 2:
                    self._warning(
                        '/SurfaceCurrent["sequencingRule.scanDirection"] attribute should have 2 values'
                    )
                elif "axisNames" in SurfaceCurrent.keys():
                    ...
                    self._log_check("111_Dev2017")
                    axisNames = SurfaceCurrent["axisNames"]
                    if (
        ...
        self._validate_axisNames(f, SurfaceCurrent)

        subgroups = set(
            [
                name
                for name, item in SurfaceCurrent.items()
                if isinstance(item, h5py.Group)
            ]
        )

        minDatasetCurrentSpeed = _get_float_attr_or_none(
            SurfaceCurrent, "minDatasetCurrentSpeed"
        )
        if (
            minDatasetCurrentSpeed is not None
            and minDatasetCurrentSpeed != -9999.0
            and minDatasetCurrentSpeed < 0
        ):
            self._warning(
                f"{SurfaceCurrent.name}: minDatasetCurrentSpeed={minDatasetCurrentSpeed} should be in [0, 99.99] range"
            )

        maxDatasetCurrentSpeed = _get_float_attr_or_none(
            SurfaceCurrent, "maxDatasetCurrentSpeed"
        )
        if maxDatasetCurrentSpeed is not None and maxDatasetCurrentSpeed > 99.99:
            self._warning(
                f"{SurfaceCurrent.name}: maxDatasetCurrentSpeed={maxDatasetCurrentSpeed} should be in [0, 99.99] range"
            )

        if (
            minDatasetCurrentSpeed is not None
            and maxDatasetCurrentSpeed is not None
            and minDatasetCurrentSpeed != -9999.0
            and maxDatasetCurrentSpeed != -9999.0
            and minDatasetCurrentSpeed > maxDatasetCurrentSpeed
        ):
            self._warning(
                f"{SurfaceCurrent.name}: minDatasetCurrentSpeed={minDatasetCurrentSpeed} > maxDatasetCurrentSpeed={maxDatasetCurrentSpeed}"
            )

        self._log_check("111_Dev2013")
        if len(subgroups) == 0:
            self._critical_error("/SurfaceCurrent has no groups")
        else:
            for i in range(1, len(subgroups) + 1):
                expected_name = "SurfaceCurrent.%02d" % i
                if expected_name not in subgroups:
                    self._critical_error(
                        "/SurfaceCurrent/{expected_name} group does not exist"
                    )

            for name in subgroups:
                if not name.startswith("SurfaceCurrent."):
                    self._warning(
                        "/SurfaceCurrent/{expected_name} is an unexpected group"
                    )

        self._log_check("111_Dev2014")
        if numInstances and len(subgroups) != numInstances:
            self._critical_error(
                f"/SurfaceCurrent has {len(subgroups)} groups whereas numInstances={numInstances}"
            )

        self._log_check("111_Dev2015")
        self._validate_sequencingRuleType(SurfaceCurrent)

        # Attributes and groups already checked above
        self._log_check("111_Dev2021")
        for name, item in SurfaceCurrent.items():
            if isinstance(item, h5py.Dataset) and name != "axisNames":
                self._warning(f"/SurfaceCurrent has unexpected dataset {name}")

            if isinstance(item, h5py.Group) and name.startswith("SurfaceCurrent."):
                self._validate_SurfaceCurrent_instance(f, SurfaceCurrent, item)
    ...
    def _validate_SurfaceCurrent_instance(self, f, SurfaceCurrent, instance):
        ...
        self._log_check("111_Dev3001")
        self._log_check("111_Dev3005")
        self._log_check("111_Dev3019")
        self._log_check("111_Dev3020")
        self._check_attributes(
            f"SurfaceCurrent feature instance group {instance.name}",
            instance,
            attr_list,
        )

        self._log_check("111_Dev3021")
        countGroups = 0
        for name, item in instance.items():
            if isinstance(item, h5py.Dataset) and name not in (
                ...
                self._warning(
                    f"SurfaceCurrent feature instance group {instance.name} has unexpected dataset '{name}'"
                )
                ...
                if name.startswith("Group_"):
                    ...
                    self._warning(
                        f"SurfaceCurrent feature instance group {instance.name} has unexpected group '{name}'"
                    )
        ...
        if (
            ...
            if isinstance(dateTimeOfLastRecord, str) and isinstance(
                ...
                self._log_check("111_Dev3006")
                if dateTimeOfLastRecord < dateTimeOfFirstRecord:
                    self._error(
                        f"SurfaceCurrent feature instance group {instance.name}: dateTimeOfLastRecord < dateTimeOfFirstRecord"
                    )
            else:
        ...
        if numGRP is not None:
            self._log_check("111_Dev3007")
            if numGRP <= 0:
                self._error(
                    f"SurfaceCurrent feature instance group {instance.name}: numGRP is <= 0"
                )
            if dataCodingFormat == 2:
                self._log_check("111_Dev3023")
                if numGRP != countGroups:
                    self._error(
                        f"SurfaceCurrent feature instance group {instance.name}: Count of values groups does not match attribute numGRP in instance group"
                    )

        numberOfTimes = _get_int_attr_or_none(instance, "numberOfTimes")
        if numberOfTimes is not None:
            self._log_check("111_Dev3003")
            if numberOfTimes <= 0:
                self._error(
                    f"SurfaceCurrent feature instance group {instance.name}: numberOfTimes is <= 0"
                )
            if dataCodingFormat == 2:
                if numGRP is not None and numberOfTimes != numGRP:
                    self._error(
                        f"SurfaceCurrent feature instance group {instance.name}: numberOfTimes is different from numGRP"
                    )

        timeRecordInterval = _get_int_attr_or_none(instance, "timeRecordInterval")
        if timeRecordInterval is not None:
            self._log_check("111_Dev3004")
            if timeRecordInterval <= 0:
                self._critical_error(
                    f"SurfaceCurrent feature instance group {instance.name}: timeRecordInterval is <= 0"
                )
            elif (
                ...
                if computedNumberOfTimes != numberOfTimes:
                    self._warning(
                        f"SurfaceCurrent feature instance group {instance.name}: given dateTimeOfFirstRecord, dateTimeOfFirstRecord and timeRecordInterval, the number of groups should be {computedNumberOfTimes} whereas it is {numberOfTimes}"
                    )
        ...
        if present and missing:
            self._critical_error(
                f"SurfaceCurrent feature instance group {instance.name}: attributes {present} are present, but {missing} are missing"
            )
            ...
            if horizontalCRS and horizontalCRS > 0:
                if self._is_geographic_2D(f):
                    ...
                    if (
                        ...
                        self._log_check("111_Dev3002")
                        if westBoundLongitude < top_westBoundLongitude:
                            self._error(
                                f"SurfaceCurrent feature instance group {instance.name}: westBoundLongitude={westBoundLongitude} < top_westBoundLongitude={top_westBoundLongitude}"
                            )
                        ...
                        if eastBoundLongitude > top_eastBoundLongitude:
                            self._error(
                                f"SurfaceCurrent feature instance group {instance.name}: eastBoundLongitude={eastBoundLongitude} > top_eastBoundLongitude={top_eastBoundLongitude}"
                            )
                        if northBoundLatitude > top_northBoundLatitude:
                            self._error(
                                f"SurfaceCurrent feature instance group {instance.name}: northBoundLatitude={northBoundLatitude} > top_northBoundLatitude={top_northBoundLatitude}"
                            )
                    ...
                    if (
                        ...
                        self._error(
                            f"SurfaceCurrent feature instance group {instance.name}: westBoundLongitude, eastBoundLongitude, northBoundLatitude, southBoundLatitude are longitudes/latitudes whereas they should be projected coordinates, given the horizontalCRS is projected"
                        )
                    ...
                    if gdal_available:
                        ...
                        if westLon + epsilon < crs_area_of_use.west_lon_degree:
                            self._error(
                                f"SurfaceCurrent feature instance group {instance.name}: westLon={westLon} < crs_area_of_use.west_lon_degree={crs_area_of_use.west_lon_degree}"
                            )
                        if southLat + epsilon < crs_area_of_use.south_lat_degree:
                            self._error(
                                f"SurfaceCurrent feature instance group {instance.name}: southLat={southLat} < crs_area_of_use.south_lat_degree={crs_area_of_use.south_lat_degree}"
                            )
                        if eastLon - epsilon > crs_area_of_use.east_lon_degree:
                            self._error(
                                f"SurfaceCurrent feature instance group {instance.name}: eastLon={eastLon} > crs_area_of_use.east_lon_degree={crs_area_of_use.east_lon_degree}"
                            )
                        if northLat - epsilon > crs_area_of_use.north_lat_degree:
                            self._error(
                                f"SurfaceCurrent feature instance group {instance.name}: northLat={northLat} > crs_area_of_use.north_lat_degree={crs_area_of_use.north_lat_degree}"
                            )
                        ...
                        if (
                            ...
                            if westLon + epsilon < top_westBoundLongitude:
                                self._error(
                                    f"SurfaceCurrent feature instance group {instance.name}: westBoundLongitude={westLon} ({westBoundLongitude}) < top_westBoundLongitude={top_westBoundLongitude}"
                                )
                            ...
                            if eastLon - epsilon > top_eastBoundLongitude:
                                self._error(
                                    f"SurfaceCurrent feature instance group {instance.name}: eastBoundLongitude={eastLon} ({eastBoundLongitude}) > top_eastBoundLongitude={top_eastBoundLongitude}"
                                )
                            if northLat - epsilon > top_northBoundLatitude:
                                self._error(
                                    f"SurfaceCurrent feature instance group {instance.name}: northBoundLatitude={northLat} ({northBoundLatitude}) > top_northBoundLatitude={top_northBoundLatitude}"
                                )

                    else:
                        self._warning(
                            "Test checking consistency of bounds in SurfaceCurrent feature instance group compared to top level attributes skipped due to GDAL not available"
                        )

            if eastBoundLongitude <= westBoundLongitude:
                self._error(
                    f"SurfaceCurrent feature instance group {instance.name}: eastBoundLongitude <= westBoundLongitude"
                )
            if northBoundLatitude <= southBoundLatitude:
                self._error(
                    f"SurfaceCurrent feature instance group {instance.name}: northBoundLatitude <= southBoundLatitude"
                )
        ...
        if gridOriginLongitude is not None and gridOriginLatitude is not None:
            ...
            if (
                ...
                if (
                    ...
                    self._error(
                        f"SurfaceCurrent feature instance group {instance.name}: gridOriginLongitude={gridOriginLongitude} < westBoundLongitude={westBoundLongitude}"
                    )
                if (
                    gridOriginLongitude - 1e-6 * abs(gridOriginLongitude)
                    > eastBoundLongitude
                    ...
                    self._error(
                        f"SurfaceCurrent feature instance group {instance.name}: gridOriginLongitude={gridOriginLongitude} > eastBoundLongitude={eastBoundLongitude}"
                    )
                if (
                    ...
                    self._error(
                        f"SurfaceCurrent feature instance group {instance.name}: gridOriginLatitude={gridOriginLatitude} < southBoundLatitude={southBoundLatitude}"
                    )
                if (
                    gridOriginLatitude - 1e-6 * abs(gridOriginLatitude)
                    > northBoundLatitude
                    ...
                    self._error(
                        f"SurfaceCurrent feature instance group {instance.name}: gridOriginLatitude={gridOriginLatitude} > northBoundLatitude={northBoundLatitude}"
                    )
                ...
                if gdal_available and horizontalCRS > 0:
                    ...
                    if origin_long + epsilon < crs_area_of_use.west_lon_degree:
                        self._error(
                            f"SurfaceCurrent feature instance group {instance.name}: origin_long={origin_long} < crs_area_of_use.west_lon_degree={crs_area_of_use.west_lon_degree}"
                        )
                    if origin_lat + epsilon < crs_area_of_use.south_lat_degree:
                        self._error(
                            f"SurfaceCurrent feature instance group {instance.name}: origin_lat={origin_lat} < crs_area_of_use.south_lat_degree={crs_area_of_use.south_lat_degree}"
                        )
                    if origin_long - epsilon > crs_area_of_use.east_lon_degree:
                        self._error(
                            f"SurfaceCurrent feature instance group {instance.name}: origin_long={origin_long} > crs_area_of_use.east_lon_degree={crs_area_of_use.east_lon_degree}"
                        )
                    if origin_lat - epsilon > crs_area_of_use.north_lat_degree:
                        self._error(
                            f"SurfaceCurrent feature instance group {instance.name}: origin_lat={origin_lat} > crs_area_of_use.north_lat_degree={crs_area_of_use.north_lat_degree}"
                        )

        self._log_check("111_Dev3010")
        ...
        if gridSpacingLongitudinal is not None and gridSpacingLongitudinal <= 0:
            self._critical_error(
                f"SurfaceCurrent feature instance group {instance.name}: Grid spacing attribute in instance group has value out of range: gridSpacingLongitudinal <= 0"
            )
        ...
        if gridSpacingLatitudinal is not None and gridSpacingLatitudinal <= 0:
            self._critical_error(
                f"SurfaceCurrent feature instance group {instance.name}: Grid spacing attribute in instance group has value out of range: gridSpacingLatitudinal <= 0"
            )

        self._log_check("111_Dev3011")
        if (
            ...
            self._warning(
                f"SurfaceCurrent feature instance group {instance.name}: Value of gridSpacingLongitudinal or gridSpacingLatitudinal in instance group too high: gridSpacingLongitudinal={gridSpacingLongitudinal} > 0.5 * (eastBoundLongitude - westBoundLongitude)={0.5 * (eastBoundLongitude - westBoundLongitude)}"
            )
        ...
        if (
            ...
            self._warning(
                f"SurfaceCurrent feature instance group {instance.name}: Value of gridSpacingLongitudinal or gridSpacingLatitudinal in instance group too high: gridSpacingLatitudinal={gridSpacingLatitudinal} > 0.5 * (northBoundLatitude - southBoundLatitude)={0.5 * (northBoundLatitude - southBoundLatitude)}"
            )

        self._log_check("111_Dev3012")
        numPointsLongitudinal = _get_int_attr_or_none(instance, "numPointsLongitudinal")
        if numPointsLongitudinal < 1:
            self._critical_error(
                f"SurfaceCurrent feature instance group {instance.name}: Grid must be at least 1X1: numPointsLongitudinal < 1"
            )
        ...
        if numPointsLatitudinal < 1:
            self._critical_error(
                f"SurfaceCurrent feature instance group {instance.name}: Grid must be at least 1X1: numPointsLatitudinal < 1"
            )

        self._log_check("111_Dev3013")
        if (
            gridSpacingLongitudinal is not None
            and eastBoundLongitude is not None
            and westBoundLongitude is not None
            ...
            self._warning(
                f"SurfaceCurrent feature instance group {instance.name}: Grid dimensions are incompatible with instance bounding box: gridSpacingLongitudinal={gridSpacingLongitudinal} != (eastBoundLongitude - westBoundLongitude) / (numPointsLongitudinal - 1)={(eastBoundLongitude - westBoundLongitude) / (numPointsLongitudinal - 1)}"
            )
        ...
        if (
            ...
            self._warning(
                f"SurfaceCurrent feature instance group {instance.name}: Grid dimensions are incompatible with instance bounding box: gridSpacingLatitudinal={gridSpacingLatitudinal} != (northBoundLatitude - southBoundLatitude) / (numPointsLatitudinal - 1)={(northBoundLatitude - southBoundLatitude) / (numPointsLatitudinal - 1)}"
            )

        self._log_check("111_Dev3014")
        # gridOriginLongitude is encoded as a float64, whereas westBoundLongitude on a float32
        # hence add some tolerance so comparison is fair
        if (
            westBoundLongitude is not None
            ...
            self._warning(
                f"SurfaceCurrent feature instance group {instance.name}: Grid origin does not coincide with instance bounding box; westBoundLongitude={westBoundLongitude} !=  gridOriginLongitude={_cast_to_float32(gridOriginLongitude)}"
            )
        ...
        if (
            ...
            self._warning(
                f"SurfaceCurrent feature instance group {instance.name}: Grid origin does not coincide with instance bounding box: southBoundLatitude={southBoundLatitude} !=  gridOriginLatitude={_cast_to_float32(gridOriginLatitude)}"
            )

        self._log_check("111_Dev3015")
        if "startSequence" in instance.attrs:
            ...
            if isinstance(startSequence, str):
                ...
                if (
                    ...
                    self._warning(
                        f"SurfaceCurrent feature instance group {instance.name}: invalid content for startSequence in instance"
                    )
                else:
        ...
        for idx_grp in range(1, numGRP + 1):
            ...
            if grp_name not in instance.keys() or not isinstance(
                ...
                self._critical_error(
                    f"SurfaceCurrent feature instance group {instance.name}: no {grp_name} subgroup"
                )
            else:
        ...
        if "uncertainty" in instance.keys() and isinstance(
            ...
            if uncertainty.shape != (2,):
                self._critical_error(
                    f"{instance.name}/uncertainty' is not a one-dimensional dataset of shape 2"
                )
    ...
    def _validate_Group_XXX(
        ...
        dataCodingFormat = _get_int_attr_or_none(
            f["SurfaceCurrent"], "dataCodingFormat"
        )

        # Cf Table 12-4 - Values Group attributes
        if dataCodingFormat in (1, 2, 3, 4):
            attr_list = [
                AttributeDefinition(
                    name="timePoint",
                    required=True,
        ...
        if dataCodingFormat == 8:
            attr_list += [
                AttributeDefinition(
                    name="dataCodingFormatlocationMRN",
                    required=False,
                    type="string",
                    fixed_value=None,
                ),
    ...
    def _validate_values(
        ...
        self._log_check("111_Dev5012")
        Group_F_SurfaceCurrent = None
        if "Group_F" in f:
            Group_F = f["Group_F"]
            if isinstance(Group_F, h5py.Group) and "SurfaceCurrent" in Group_F:
                Group_F_SurfaceCurrent = Group_F["SurfaceCurrent"]
                if (
                    isinstance(Group_F_SurfaceCurrent, h5py.Dataset)
                    and len(Group_F_SurfaceCurrent.shape) == 1
                ):
                    num_components = None
                    if num_components and values_type.get_nmembers() != num_components:
                        self._critical_error(
                            f"{values.name} type has {values_type.get_nmembers()} members whereas {num_components} are expected from /Group_F/SurfaceCurrent"
                        )
                        return
                else:
                    Group_F_SurfaceCurrent = None

        # Check consistency between "values" and "/Group_F/SurfaceCurrent"
        found_surfaceCurrentSpeed = False
        found_surfaceCurrentDirection = False
        found_speedUncertainty = False
        found_directionUncertainty = False
        for member_idx in range(values_type.get_nmembers()):
            ...
            if Group_F_SurfaceCurrent:
                expected = Group_F_SurfaceCurrent[member_idx][0]
                if component_name != expected:
                    self._critical_error(
                        f"{values.name} member {member_idx} name = {component_name} is not Group_F_SurfaceCurrent[{member_idx}]['name']] = {expected}"
                    )
            assert isinstance(component_name, bytes)
            if component_name == b"surfaceCurrentSpeed":
                found_surfaceCurrentSpeed = True
                if not self._is_float32(subtype):
                    self._critical_error(
                        f"{values.name} member {component_name} is not a float32"
                    )
            elif component_name == b"surfaceCurrentDirection":
                found_surfaceCurrentDirection = True
                if not self._is_float32(subtype):
                    self._critical_error(
                        f"{values.name} member {component_name} is not a float32"
                    )
            elif component_name == b"speedUncertainty":
                found_speedUncertainty = True
                if not self._is_float32(subtype):
                    self._critical_error(
                        f"{values.name} member {component_name} is not a float32"
                    )
            elif component_name == b"directionUncertainty":
                found_directionUncertainty = True
                if not self._is_float32(subtype):
                    self._critical_error(
                        f"{values.name} member {component_name} is not a float32"
                    )
        ...
        minDatasetCurrentSpeed = _get_float_attr_or_none(
            f["SurfaceCurrent"], "minDatasetCurrentSpeed"
        )
        maxDatasetCurrentSpeed = _get_float_attr_or_none(
            f["SurfaceCurrent"], "maxDatasetCurrentSpeed"
        )
        if found_surfaceCurrentSpeed:
            masked_height = np.ma.masked_equal(values[:]["surfaceCurrentSpeed"], -9999)
            actualMinSpeed = masked_height.min()
            actualMaxSpeed = masked_height.max()

            if (
                minDatasetCurrentSpeed is not None
                and maxDatasetCurrentSpeed is not None
                and minDatasetCurrentSpeed != -9999
                and maxDatasetCurrentSpeed != -9999
                and minDatasetCurrentSpeed <= maxDatasetCurrentSpeed
            ):
                self._log_check("111_Dev5013")
                if actualMinSpeed < minDatasetCurrentSpeed:
                    self._error(
                        f"{values.name} : minimum surfaceCurrentSpeed is {actualMinSpeed}, whereas minDatasetCurrentSpeed attribute = {minDatasetCurrentSpeed}"
                    )

                if actualMaxSpeed > maxDatasetCurrentSpeed:
                    self._error(
                        f"{values.name} : maximum surfaceCurrentSpeed is {actualMaxSpeed}, whereas maxDatasetCurrentSpeed attribute = {maxDatasetCurrentSpeed}"
                    )

            self._log_check("111_Dev5014")
            if actualMinSpeed < 0:
                self._error(
                    f"{values.name} : minimum surfaceCurrentSpeed is {actualMinSpeed}, whereas it should be >= 0"
                )

            if actualMaxSpeed > 99.0:
                self._error(
                    f"{values.name} : maximum surfaceCurrentSpeed is {actualMaxSpeed}, whereas it should be <= 99"
                )

        if found_surfaceCurrentDirection:
            masked_direction = np.ma.masked_equal(
                values[:]["surfaceCurrentDirection"], -9999
            )
            self._log_check("111_Dev5015")
            actualMin = masked_direction.min()
            if actualMin < 0:
                self._error(
                    f"{values.name} : minimum surfaceCurrentDirection is {actualMin}, whereas it should be >= 0"
                )
            actualMax = masked_direction.max()
            if actualMax > 359.9:
                self._error(
                    f"{values.name} : maximum surfaceCurrentDirection is {actualMax}, whereas it should be <= 359.9"
                )

        if found_speedUncertainty:
            self._log_check("111_Dev5016")
            masked_uncertainty = np.ma.masked_equal(values[:]["speedUncertainty"], -1.0)
            actualMin = masked_uncertainty.min()
            if actualMin < 0:
                self._error(
                    f"{values.name} : minimum speedUncertainty is {actualMin}, whereas it should be >= 0"
                )
            actualMax = masked_uncertainty.max()
            if actualMax > 99.0:
                self._error(
                    f"{values.name} : maximum speedUncertainty is {actualMax}, whereas it should be <= 99"
                )

        if found_directionUncertainty:
            self._log_check("111_Dev5016")

            masked_uncertainty = np.ma.masked_equal(
                values[:]["directionUncertainty"], -1.0
            )
            actualMin = masked_uncertainty.min()
            if actualMin < 0:
                self._error(
                    f"{values.name} : minimum directionUncertainty is {actualMin}, whereas it should be >= 0"
                )
            actualMax = masked_uncertainty.max()
            if actualMax > 359.9:
                self._error(
                    f"{values.name} : maximum directionUncertainty is {actualMax}, whereas it should be <= 359.9"
                )
...
def usage():
    print("Usage: validate_s111.py [-q] <filename>")
    print("")
    print("Validates a S111 files against the Edition 2.0 specification.")
    print("")
